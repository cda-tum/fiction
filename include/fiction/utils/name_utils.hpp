...
/**
 * Same as the other restore_signal_names function but this overload uses a node_map with a branching_signal_container
 * that is specifically used for networks or layouts that allow branches to be distinct, e.g., by their position on the
 * layout.
 *
 * @tparam NtkSrc Source network type.
 * @tparam NtkDest Target network type.
