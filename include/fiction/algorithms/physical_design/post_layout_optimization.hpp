                ...
                layout.foreach_node(
                    [&layout, &gate_tiles](const auto& node)
                    {
                        if (const tile<Lyt> tile = layout.get_tile(node);
                            layout.is_inv(node) || layout.is_and(node) || layout.is_nand(node) || layout.is_xor(node) ||
                            layout.is_xnor(node) || layout.is_fanout(node) || layout.is_or(node) ||
                            layout.is_nor(node) || layout.is_pi_tile(tile) || layout.is_po_tile(tile))
                        {
