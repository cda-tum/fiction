...
class mugen_handler
{
  public:
     ...
     */
    [[nodiscard]] bool skippable(const aspect_ratio_type_t<Lyt>& ratio) const noexcept
    {
     ...
     */
    void update_aspect_ratio(const aspect_ratio_type_t<Lyt>& ratio) noexcept
    {
        lyt.resize(ratio);
    }
    /**
...
class one_pass_synthesis_impl
{
  public:
    ...
    std::optional<Lyt> run()
    {
             ...
             ++ari)  // <= to prevent overflow
        {
            ...
            const auto aspect_ratio = aspect_ratio_type_t<Lyt>{(*ari).x(), (*ari).y(), ps.crossings ? 1 : 0};
  ...
  private:
     ...
     */
    aspect_ratio_iterator<aspect_ratio_type_t<Lyt>> ari{0};
