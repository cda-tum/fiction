            ...
            [[nodiscard]] std::vector<population_stability_information<Lyt>> run() noexcept {
                ...
                for (const auto &energy_and_index: energy_and_unique_charge_index) {
                    const auto it = std::find_if(
                            simulation_results.charge_distributions.begin(),
                            simulation_results.charge_distributions.end(),
                                ...
                                switch (charge_lyt.get_charge_state(c)) {
                                    case sidb_charge_state::NEGATIVE: {
                                        population_stability_info = handle_negative_charges(
                                                *charge_lyt.get_local_potential(c), c,
                    ...
                    for (const auto &transition: population_stability_info.transition_from_to_with_cell_and_required_pot) {
                        population_stability_info.distance_corresponding_to_potential[transition.first] =
                                convert_potential_to_distance(transition.second.second, params.physical_parameters,
 ...
 */
    ...
    assess_physical_population_stability(const Lyt &lyt,
                                         const assess_physical_population_stability_params &params) noexcept {
