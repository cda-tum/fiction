...
class clustercomplete_impl
{
  ...
  private:
     ...
     */
    [[nodiscard]] bool
    meets_population_stability_criterion(const sidb::model::clustering_state& clustering_state) const noexcept
    {
        for (const sidb::model::cluster_projector_state_ptr& pst : clustering_state.proj_states)
        {
            ...
            for (const auto sidb_ix : pst->cluster->sidbs)
            {
                const double recv_pot_lb =
                    clustering_state.pot_bounds.get<sidb::model::bound_direction::LOWER>(sidb_ix);
                const double recv_pot_ub =
                    clustering_state.pot_bounds.get<sidb::model::bound_direction::UPPER>(sidb_ix);
     ...
     */
    void add_if_configuration_stability_is_met(const sidb::model::clustering_state& clustering_state) noexcept
    {
        ...
        for (const auto& pst : clustering_state.proj_states)
        {
            ...
            charge_layout_copy.assign_local_internal_potential_by_index(
                sidb_ix, -clustering_state.pot_bounds.get<sidb::model::bound_direction::LOWER>(sidb_ix) -
     ...
     */
    static sidb::model::cluster_projector_state_ptr take_parent_out(sidb::model::clustering_state& clustering_state,
                                                                    const uint64_t parent_pst_ix) noexcept
    {
        ...
        // move out
        sidb::model::cluster_projector_state_ptr parent_pst = std::move(clustering_state.proj_states.back());
     ...
     * given index.
     */
    static void add_parent(sidb::model::clustering_state& clustering_state, const uint64_t parent_pst_ix,
     ...
     */
    static void add_composition(sidb::model::clustering_state&               clustering_state,
                                const sidb::model::charge_space_composition& composition) noexcept
    {
        ...
        for (const sidb::model::cluster_projector_state& child_pst : composition.proj_states)
        {
            ...
            clustering_state.proj_states.emplace_back(
                std::make_unique<sidb::model::cluster_projector_state>(child_pst));
        }
    }
    /**
     ...
     */
    static void remove_composition(sidb::model::clustering_state&               clustering_state,
                                   const sidb::model::charge_space_composition& composition) noexcept
    {
     ...
     */
    void add_physically_valid_charge_configurations(sidb::model::clustering_state& clustering_state) noexcept
    {
        // check for pruning
        ...
        sidb::model::cluster_projector_state_ptr max_pst = take_parent_out(clustering_state, max_pst_ix);
    ...
    collect_physically_valid_charge_distributions_single_threaded(const sidb::model::cluster_ptr& top_cluster) noexcept
    {
        for (const sidb::model::cluster_charge_state& ccs : top_cluster->charge_space)
        {
            for (const sidb::model::charge_space_composition& composition : ccs.compositions)
            {
                ...
                sidb::model::clustering_state clustering_state{charge_layout.num_cells()};
    ...
    struct worker_queue
    {
         ...
         */
        void initialize_queue_after_stealing(const sidb::model::clustering_state& clustering_state) noexcept
        {
    ...
    struct worker
    {
         ...
         */
        sidb::model::clustering_state clustering_state;
        /**
        ...
        [[nodiscard]] std::optional<work_t> obtain_work() noexcept
        {
            ...
            while (encountered_locked_queue)
            {
                ...
                for (uint64_t i = 0; i < all_workers.size(); ++i)
                {
                    ...
                    {
                        clustering_state =
                            std::move(std::get<std::pair<sidb::model::clustering_state, work_t>>(work).first);
                                               ...
                                               const sidb::model::charge_space_composition& composition) noexcept
    {
        ...
        // un-apply max_pst, thereby making space for specialization
        sidb::model::cluster_projector_state_ptr max_pst = take_parent_out(w.clustering_state, max_pst_ix);
