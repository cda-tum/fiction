...
class critical_temperature_impl
{
  public:
    ...
    critical_temperature_impl(const std::vector<Lyt>& input_pattern_lyts, const critical_temperature_params& ps,
                              critical_temperature_stats& st, const std::vector<bdl_pair<cell<Lyt>>>& output_pairs,
                              const std::vector<bdl_wire<Lyt>>& input_wires,
                              const std::vector<bdl_wire<Lyt>>& output_wires) :
    ...
    void gate_based_simulation(const std::vector<TT>& spec) noexcept
    {
        ...
        if (layout.num_cells() > 1)
        {
            ...
            auto detected_input_bdl_wires  = std::vector<bdl_wire<Lyt>>{};
            auto detected_output_bdl_wires = std::vector<bdl_wire<Lyt>>{};
  ...
  private:
     ...
     */
    const std::vector<bdl_pair<cell<Lyt>>>* pre_detected_output_bdl_pairs{nullptr};
    /**
     * Pre-detected input BDL wires, or `nullptr` if they are to be detected here. Not owned by this object.
     */
    const std::vector<bdl_wire<Lyt>>* pre_detected_input_bdl_wires{nullptr};
    /**
     * Pre-detected output BDL wires, or `nullptr` if they are to be detected here. Not owned by this object.
     */
    const std::vector<bdl_wire<Lyt>>* pre_detected_output_bdl_wires{nullptr};
     ...
     */
    [[nodiscard]] sidb_simulation_result<Lyt> physical_simulation_of_layout(const Lyt& lyt_with_input_pattern) noexcept
    {
