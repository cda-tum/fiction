               ...
               const std::vector<bdl_wire<Lyt>>& output_bdl_wire, const std::optional<Lyt>& canvas_lyt = std::nullopt)
{
    ...
    if (canvas_lyt.has_value())
    {
        detail::is_operational_impl<Lyt, TT> p{input_pattern_layouts, spec, params, input_bdl_wire, output_bdl_wire,
                                               canvas_lyt.value()};
    ...
    for (const auto& c : input_pattern_layouts.front().get_cells_by_type(technology<Lyt>::cell_type::LOGIC))
    {
        c_lyt.assign_cell_type(c, technology<Lyt>::cell_type::LOGIC);
    }

    detail::is_operational_impl<Lyt, TT> p{input_pattern_layouts, spec, params, input_bdl_wire, output_bdl_wire, c_lyt};
