...
struct bdl_input_iterator_params
{
     ...
     *    by positioning the perturber farther away.
     */
    enum class input_bdl_configuration : uint8_t
    {
        ...
        ZERO_VIA_PERTURBER,
        /**
         * An input of `1` is generated by placing a perturber closer to the target, whereas a `0` is produced
         * by positioning the perturber farther away.
         */
        ZERO_VIA_NO_PERTURBER
    };
    /**
     * The `input_bdl_config` member allows selection between different modes for handling BDLs.
     */
    input_bdl_configuration input_bdl_config{input_bdl_configuration::ZERO_VIA_PERTURBER};
    /**
     * The `detect_bdl_pairs` member holds the settings required to identify and handle pairs of BDLs during processing.
     * It encapsulates various thresholds and criteria specific to the detection of BDL pairs.
     */
  ...
  private:
    ...
    void set_all_inputs() noexcept
    {
        for (uint64_t i = 0; i < input_pairs.size(); ++i)
        {
            ...
            {
                if (params.input_bdl_config == bdl_input_iterator_params::input_bdl_configuration::ZERO_VIA_PERTURBER)
                {
                    // set input i to 0
