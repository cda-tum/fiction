...
{
  public:
    node_duplication_planarization_impl(const NtkSrc& src, const node_duplication_params p) :
            ntk(convert_network<NtkDest>(src))
    {
        if (!is_fanout_substituted(ntk)) {
            ntk = fanout_substitution<NtkDest>(ntk);   // Apply fanout substitution if needed
        }
    }
 ...
 */
template <typename NtkDest, typename NtkSrc>
NtkDest node_duplication_planarization(const NtkSrc& ntk_src, node_duplication_params ps = {})
{
    ...
    detail::node_duplication_planarization_impl<NtkDest, NtkSrc> p{ntk_src, ps};
