...
class exact_impl
{
  public:
    exact_impl(mockturtle::names_view<networks::technology_network>& src, exact_physical_design_params p,
               exact_physical_design_stats& st, const sidb::surface_black_list<Lyt, fcn::port_direction>& sbl = {}) :
            ps{std::move(p)},
            pst{st},
  ...
  private:
     ...
     */
    const sidb::surface_black_list<Lyt, fcn::port_direction> black_list;
    /**
    ...
    class smt_handler
    {
      public:
        ...
        smt_handler(ctx_ptr ctxp, Lyt& lyt, const topology_ntk_t& ntk, exact_physical_design_params ps,
                    const sidb::surface_black_list<Lyt, fcn::port_direction>& sbl) noexcept :
                ctx{std::move(ctxp)},
                layout{lyt},
                network{ntk},
                params{std::move(ps)},
                ...
                node2pos{ntk},
                depth_ntk{ntk},
                inv_levels{networks::inverse_levels(ntk)}
        {}
        /**
      ...
      private:
                    ...
                    network.foreach_node(
                        [this, &t](const auto& n)
                        {
                            if (!skip_const_or_io_node(n))
                            {
                                ...
                                networks::foreach_outgoing_edge(
                                    network, n,
                    ...
                    network.foreach_node(
                        [this, &t](const auto& n)
                        {
                            if (!skip_const_or_io_node(n))
                            {
                                ...
                                networks::foreach_incoming_edge(
                                    network, n,
        ...
        void global_synchronization()
        {
            ...
            {
                const auto paths = networks::all_incoming_edge_paths(network, n);
                ...
                for (const auto& p : paths)
                {
                    ...
                    for (const auto& e : p)
                    {
                        ...
                        {
                            if (networks::has_incoming_primary_input(network, src))
                            {
                                path_length.push_back(ncl_to_real(src));
                            }
                        }
            ...
            if (params.border_io && (layout.is_clocking_scheme(layouts::clocking::TWODDWAVE_NAME) ||
                                     layout.is_clocking_scheme(layouts::clocking::TWODDWAVE_HEX_NAME)))
            {
                if (params.io_pins)
                {
            ...
            else if (params.border_io && (layout.is_clocking_scheme(layouts::clocking::COLUMNAR_NAME) ||
                                          layout.is_clocking_scheme(layouts::clocking::ROW_NAME)))
            {
            ...
            layout.foreach_ground_tile(
                [this](const auto& t)
                {
                    if (layout.is_at_eastern_border(t) || layout.is_at_southern_border(t) || is_updated_tile(t))
                    {
                        ...
                        {
                            const auto tile_degree = layouts::num_adjacent_coordinates(layout, t);
        ...
        void utilize_hierarchical_information()
        {
            ...
            if (layout.is_clocking_scheme(layouts::clocking::COLUMNAR_NAME))
            {
                            ...
                            if (!skip_const_or_io_node(n))
                            {
                                     ...
                                     column < std::min(static_cast<decltype(layout.y())>(l), layout.x()); ++column)
                                {
                                    for (auto row = 0u; row <= layout.y(); ++row)
                                    {
                                        if (const auto t = typename Lyt::tile{column, row}; is_added_tile(t))
                                        {
                                            ...
                                            networks::foreach_outgoing_edge(network, n,
                                                                            [this, &t](const auto& e)
                                                                            {
                                                                                if (!skip_const_or_io_edge(e))
                                                                                {
                                ...
                                for (auto column = layout.x() - il + 1; column < layout.x(); ++column)
                                {
                                    for (auto row = 0u; row <= layout.y(); ++row)
                                    {
                                        ...
                                        networks::foreach_incoming_edge(network, n,
                                                                        [this, &t](const auto& e)
                                                                        {
                                                                            if (!skip_const_or_io_edge(e))
                                                                            {
            ...
            if (layout.is_clocking_scheme(layouts::clocking::ROW_NAME))
            {
                // restrict node placement according to the hierarchy level
                if (params.io_pins && params.border_io)
                {
            ...
            else if (layout.is_clocking_scheme(layouts::clocking::TWODDWAVE_NAME))
            {
                                ...
                                apply_to_added_tiles(
                                    [this, &n, &l, &il](const auto& t)
                                    {
                                        if (t.x + t.y < static_cast<decltype(t.x + t.y)>(l))
                                        {
                                            ...
                                            // same for the outgoing edges
                                            networks::foreach_outgoing_edge(network, n, [this, &t](const auto& e)
                                                                            { solver->add(!(get_te(t, e))); });
                                        }
        ...
        {
            const auto output_signal = network.make_signal(networks::fanins(network, n).fanin_nodes[0]);
...
template <typename Lyt, typename Ntk>
std::optional<Lyt>
exact_with_blacklist(const Ntk& ntk, const sidb::surface_black_list<Lyt, fcn::port_direction>& black_list,
