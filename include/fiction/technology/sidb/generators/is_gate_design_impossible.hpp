 ...
 */
[[nodiscard]] inline bool is_gate_design_impossible(const layout& skeleton_with_defects,
                                                    ...
                                                    const is_gate_design_impossible_params&        params = {})
{
    ...
    assert(skeleton_with_defects.num_pis() > 0 && "lyt needs input dots");
    assert(skeleton_with_defects.num_pos() > 0 && "lyt needs output dots");

    const auto output_pairs = simulation::logic::detect_bdl_pairs(
        skeleton_with_defects, dot_tag::OUTPUT, params.bdl_iterator_params.bdl_wire_params.bdl_pairs_params);
...
[[nodiscard]] bool is_gate_design_impossible(const Lyt&                                     skeleton_with_defects,
                                             const std::vector<kitty::dynamic_truth_table>& spec,
                                             const is_gate_design_impossible_params&        params = {})
{
