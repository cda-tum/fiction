 ...
 */
[[nodiscard]] inline std::mt19937_64& random_generator()
{
 ...
 * skeleton's lattice, dots, and defects carry over; sites that hold a defect or that a neutral defect affects are
                                                                  ...
                                                                  const std::optional<layout>& skeleton = std::nullopt)
{
    ...
    {
        lyt = *skeleton;
        number_of_sidbs_of_final_layout += lyt.num_dots();
    ...
    {
        if (lyt.num_dots() == number_of_sidbs_of_final_layout)
        {
        ...
        if (sidbs_affected_by_defects.contains(random_site) ||
            lyt.get_defect(random_site).type != model::defect_type::NONE || !lyt.is_empty_site(random_site))
        {
            continue;
        }

        lyt.assign_sidb(random_site, tag);
