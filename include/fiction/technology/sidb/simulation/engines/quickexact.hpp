...
#ifndef FICTION_TECHNOLOGY_SIDB_SIMULATION_ENGINES_QUICKEXACT_HPP
#define FICTION_TECHNOLOGY_SIDB_SIMULATION_ENGINES_QUICKEXACT_HPP
...
class quickexact_impl
{
  public:
    ...
    sidb::simulation::result<Lyt> run() noexcept
    {
        ...
        mockturtle::stopwatch<>::duration time_counter{};
        {
            ...
            if (number_of_sidbs > 1)
            {
                ...
                {
                    sidb::surfaces::charge_distribution_surface charge_layout{layout};
                ...
                {
                    sim_result.charge_distributions.emplace_back(charge_lyt);
                }
            }
            ...
            else if (number_of_sidbs == 1)
            {
                ...
                while (charge_lyt.get_charge_index_and_base().first < charge_lyt.get_max_charge_index())
                {
                    ...
                    charge_lyt.increase_charge_index_by_one(
                        sidb::surfaces::dependent_cell_mode::
                            VARIABLE);  // `sidb::surfaces::dependent_cell_mode::VARIABLE`
  ...
  private:
    ...
    void conduct_simulation(ChargeLyt& charge_layout, const required_simulation_base_number base_number) noexcept
    {
        ...
        charge_layout.update_after_charge_change(sidb::surfaces::dependent_cell_mode::FIXED);
        ...
        if constexpr (is_sidb_defect_surface_v<Lyt> && is_charge_distribution_surface_v<Lyt>)
        {
            layout.foreach_sidb_defect(
                [this, &charge_layout](const auto& cd)
                {
                    ...
                    {
                        charge_layout.add_sidb_defect_to_potential_landscape(cell, layout.get_defect(cell));
                    }
                });
        }
    ...
    void two_state_simulation(ChargeLyt& charge_layout) noexcept
    {
        ...
        for (gci = 0; gci <= charge_layout.get_max_charge_index(); ++gci)
        {
            charge_layout.assign_charge_index_by_gray_code(*gci, previous_charge_index,
                                                           sidb::surfaces::dependent_cell_mode::VARIABLE,
                                                           sidb::surfaces::energy_calculation::KEEP_OLD_ENERGY_VALUE,
                                                           sidb::surfaces::charge_distribution_history::CONSIDER);

            previous_charge_index = *gci;

            if (charge_layout.is_physically_valid())
            {
                sidb::surfaces::charge_distribution_surface<Lyt> charge_lyt_copy{charge_lyt};
        ...
        while (charge_layout.get_charge_index_and_base().first < charge_layout.get_max_charge_index())
        {
            while (charge_layout.get_charge_index_of_sub_layout() < charge_layout.get_max_charge_index_sub_layout())
            {
                ...
                charge_layout.increase_charge_index_of_sub_layout_by_one(
                    sidb::surfaces::dependent_cell_mode::VARIABLE,
                    sidb::surfaces::energy_calculation::KEEP_OLD_ENERGY_VALUE,
                    sidb::surfaces::charge_distribution_history::
                        CONSIDER);  // `sidb::surfaces::dependent_cell_mode::VARIABLE`
                                    // allows that the charge state of the
                                    // dependent cell is automatically changed
                                    // based on the new charge distribution.
            }
            ...
            charge_layout.increase_charge_index_by_one(
                sidb::surfaces::dependent_cell_mode::VARIABLE,
                sidb::surfaces::energy_calculation::
                    KEEP_OLD_ENERGY_VALUE);  // `sidb::surfaces::dependent_cell_mode::VARIABLE`
                                             // allows that the charge state of the
                                             // dependent cell is automatically changed
                                             // based on the new charge distribution.
        }
            ...
            layout.foreach_sidb_defect(
                [this](const auto& cd)
                {
                    if (const auto& [cell, defect] = cd;
                        defect.type != sidb::model::defect_type::NONE && sidb::model::is_charged_defect_type(cd.second))
                    {
                        charge_lyt.add_sidb_defect_to_potential_landscape(cell, layout.get_defect(cell));
                    }
                });
        }
 ...
 */
template <typename Lyt>
[[nodiscard]] sidb::simulation::result<Lyt> quickexact(const Lyt&                          lyt,
...
#endif  // FICTION_TECHNOLOGY_SIDB_SIMULATION_ENGINES_QUICKEXACT_HPP
