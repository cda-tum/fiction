...
{
  public:
    critical_temperature_impl(const layout& lyt, const critical_temperature_params& ps,
                              critical_temperature_stats& st) :
            lyt_{lyt},
            params{ps},
            stats{st},
            bii(logic::bdl_input_iterator{lyt_, params.operational_params.input_bdl_iterator_params}),
     ...
     * lyt_ under many parameter settings can determine them once and pass them to every call. The layouts and the
     ...
     *
     * @param input_pattern_lyts One lyt_ per input pattern, indexed by input pattern, as generated by
     ...
     * @param output_pairs Output BDL pairs of the lyt_.
     * @param input_wires BDL input wires of the lyt_.
     * @param output_wires BDL output wires of the lyt_.
     */
    ...
    /**
     * *Gate-based Critical Temperature* Simulation of a SiDB lyt_ for a given Boolean function.
     *
     * @tparam TT Type of the truth table.
     * @param spec Expected Boolean function of the lyt_ given as a multi-output truth table.
     */
    ...
    void gate_based_simulation(const std::vector<TT>& spec) noexcept
    {
        ...
        assert(lyt_.num_pis() > 0 && "gate needs input cells");
        assert(lyt_.num_pos() > 0 && "gate needs output cells");
        ...
        if (lyt_.num_cells() > 1)
        {
            ...
            const auto detected_output_bdl_pairs =
                pre_detected_output_bdl_pairs != nullptr ?
                    ...
                    sidb::simulation::logic::detect_bdl_pairs(
                        lyt_, sidb::sidb_technology::cell_type::OUTPUT,
                ...
                pre_detected_input_bdl_wires == nullptr)
            {
                detected_input_bdl_wires = sidb::simulation::logic::detect_bdl_wires(
                    lyt_, params.operational_params.input_bdl_iterator_params.bdl_wire_params,
            ...
            for (auto i = 0u; i < spec.front().num_bits(); ++i)
            {
                ...
                // if positively charged SiDBs can occur, the SiDB lyt_ is considered as non-operational
                ...
                // performs physical simulation of a given SiDB lyt_ at a given input combination
                ...
                // The energy distribution of the physically valid charge configurations for the given lyt_ is
                // determined.
    ...
    void non_gate_based_simulation() noexcept
    {
        ...
        if (params.operational_params.sim_engine == engine::QUICKEXACT)
        {
            ...
            // All physically valid charge configurations are determined for the given lyt_ (`QuickExact` simulation
            // is used to provide 100 % accuracy for the Critical Temperature).
            simulation_results = sidb::simulation::engines::quickexact(lyt_, qe_params);
        }
        ...
        else if (params.operational_params.sim_engine == engine::CLUSTERCOMPLETE)
        {
            ...
            // All physically valid charge configurations are determined for the given lyt_ (`ClusterComplete`
            // simulation is used to provide 100 % accuracy for the Critical Temperature).
            simulation_results = sidb::simulation::engines::clustercomplete(lyt_, cc_params);
        }
        ...
        else if (params.operational_params.sim_engine == engine::QUICKSIM)
        {
            ...
            // All physically valid charge configurations are determined for the given lyt_ (probabilistic ground
            // state simulation is used).
            if (const auto result = sidb::simulation::engines::quicksim(lyt_, qs_params); result.has_value())
            {
  ...
  private:
     ...
     * @param min_energy Minimal energy of all physically valid charge distributions of a given lyt_ (unit: eV).
    ...
    /**
     * Returns the lyt_ with the given input pattern applied.
     *
     ...
     * @return The lyt_ with `input_pattern` applied.
     */
    ...
    /**
     * This function conducts physical simulation of the given lyt_ (gate lyt_ with certain input combination).
     * The simulation results are stored in the `sim_result_100` variable.
     *
     * @param lyt_with_input_pattern The SiDB lyt_ with a given input combination applied.
