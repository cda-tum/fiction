...
class lattice : public Lyt
{};

template <typename LatticeOrientation, typename Lyt>
class lattice<LatticeOrientation, Lyt, true> : public Lyt
{
  public:
    explicit lattice(const Lyt& lyt) : Lyt(lyt)
    {
...
template <typename LatticeOrientation, typename Lyt>
class lattice<LatticeOrientation, Lyt, false> : public Lyt
{
  public:
    /**
     ...
     */
    explicit lattice(const aspect_ratio<Lyt>& ar, const std::string& name = "") : Lyt(ar, name)
    {
        static_assert(is_cell_level_layout_v<Lyt>, "Lyt is not a cell-level layout");
        static_assert(has_sidb_technology_v<Lyt>, "Lyt is not an SiDB layout");
    }
     ...
     */
    [[nodiscard]] lattice clone() const noexcept
    {
        lattice copy{Lyt::clone()};
