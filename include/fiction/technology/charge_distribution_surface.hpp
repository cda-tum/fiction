  ...
  public:
        ...
        const charge_distribution_history& history_mode = charge_distribution_history::NEGLECT) noexcept
    {
        if (history_mode == charge_distribution_history::NEGLECT)
        {
            ...
            {
                units::voltage::volt_t collect = 0_V;
    ...
    void adjacent_search(const double alpha, std::vector<uint64_t>& negative_indices) noexcept
    {
        ...
        if (!candidates.empty())
        {
            ...
            strg->system_energy +=
                units::energy::electron_volt_t(-(*this->get_local_potential_by_index(random_element)).value());
    ...
    bool three_state_simulation_required() noexcept
    {
        ...
        this->foreach_cell(
            [&required, this](const auto& c)
            {
                if (const auto local_pot = this->get_local_potential(c); local_pot.has_value())
                {
                    if ((-(*local_pot).value() + strg->phys_params.mu_p.value()) > -POP_STABILITY_ERR)
                    {
  ...
  private:
    ...
    void initialize_nm_distance_matrix() const noexcept
    {
        strg->nm_dist_mat = std::vector<std::vector<units::length::nanometer_t>>(
            this->num_cells(), std::vector<units::length::nanometer_t>(this->num_cells(), 0.0_nm));
    ...
    void initialize_potential_matrix() const noexcept
    {
        strg->pot_mat = std::vector<std::vector<units::voltage::volt_t>>(
            this->num_cells(), std::vector<units::voltage::volt_t>(this->num_cells(), 0.0_V));
...
charge_distribution_surface(const T&, const sidb_simulation_parameters&, const sidb_charge_state& cs,
                            ...
                            const std::unordered_map<typename T::cell, units::voltage::volt_t>& external_pot)
    -> charge_distribution_surface<T>;
