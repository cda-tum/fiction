...
class charge_distribution_surface<Lyt, false> : public Lyt
{
  public:
            ...
            if (strg->defects.find(c) == strg->defects.end())
            {
                ...
                this->foreach_cell(
                    [this, &c, &defect](const auto& c1)
                    {
                        ...
                        //                                sidb_nanometer_distance<Lyt>(*this, c1, c, strg->phys_params),
                        //                                defect) *
                        //                            defect.charge;
    ...
    void adding_defects_as_normal_sidbs() noexcept
    {
        for (const auto& defect : strg->defects)
        {
            strg->cell_charge.push_back(sign_to_charge_state(defect.second.charge));
            strg->sidb_order.push_back(defect.first);
        }
    }
