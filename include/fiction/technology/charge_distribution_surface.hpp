...
class charge_distribution_surface<Lyt, false> : public Lyt
{
  public:
    ...
    struct charge_distribution_storage
    {
      private:
         ...
         */
        using distance_matrix = std::vector<std::vector<units::length::nanometer_t>>;
        /**
         * The potential matrix is a vector of vectors storing the electrostatic potentials.
         */
        using potential_matrix = std::vector<std::vector<units::voltage::volt_t>>;
        /**
         * It is a vector that stores the local electrostatic potential.
         */
        using local_potential = std::vector<units::voltage::volt_t>;

      public:
        explicit charge_distribution_storage(
            const sidb_simulation_parameters&                                     params = sidb_simulation_parameters{},
            const std::unordered_map<typename Lyt::cell, units::voltage::volt_t>& external_potential = {},
         ...
         */
        std::unordered_map<typename Lyt::cell, units::voltage::volt_t> defect_local_pot{};
        /**
         * External electrostatic potential at each SiDB position (can be used when different potentials are applied to
         * different SiDBs).
         */
        std::unordered_map<typename Lyt::cell, units::voltage::volt_t> local_external_pot{};
        /**
         * Local (total) electrostatic potential at each SiDB position.
         */
        local_potential local_pot{};
        /**
         * Electrostatic energy of a given charge distribution.
         */
        units::energy::electron_volt_t system_energy{units::energy::electron_volt_t(0.0)};
        /**
     ...
     */
    [[nodiscard]] std::vector<std::pair<units::length::nanometer_t, units::length::nanometer_t>>
    get_all_sidb_locations_in_nm() const noexcept
    {
        std::vector<std::pair<units::length::nanometer_t, units::length::nanometer_t>> positions{};
     ...
     */
    [[nodiscard]] units::voltage::volt_t
    get_chargeless_potential_between_sidbs(const typename Lyt::cell& c1, const typename Lyt::cell& c2) const noexcept
    {
        if (const auto index1 = cell_to_index(c1), index2 = cell_to_index(c2); (index1 != -1) && (index2 != -1))
        {
     ...
     */
    [[nodiscard]] units::voltage::volt_t get_potential_between_sidbs(const typename Lyt::cell& c1,
                                                                     const typename Lyt::cell& c2) const noexcept
    {
        if (const auto index1 = cell_to_index(c1), index2 = cell_to_index(c2); (index1 != -1) && (index2 != -1))
        {
     ...
     * @return The potential between `index1` and `index2`.
     */
    [[nodiscard]] units::voltage::volt_t potential_between_sidbs_by_indices(const uint64_t index1,
                                                                            const uint64_t index2) const noexcept
    {
        if (strg->nm_dist_mat[index1][index2] == units::length::nanometer_t(0.0))
        {
     ...
     * returned.
     */
    std::optional<units::voltage::volt_t> get_local_potential(const typename Lyt::cell& c) const noexcept
    {
        if (const auto index = cell_to_index(c); index != -1)
        {
     ...
     */
    [[nodiscard]] std::optional<units::voltage::volt_t>
    get_local_potential_by_index(const uint64_t index) const noexcept
    {
    ...
    {
        strg->system_energy = units::energy::electron_volt_t(0.0);
    }
    /**
    ...
    void recompute_system_energy() noexcept
    {
        ...
        for (const auto& [cell, pot] : strg->defect_local_pot)
        {
            defect_energy += units::energy::electron_volt_t{
                pot * static_cast<double>(charge_state_to_sign(strg->cell_charge[cell_to_index(cell)]))};
        }
        ...
        for (const auto& [cell1, defect1] : strg->defects)
        {
            ...
            {
                defect_interaction += units::energy::electron_volt_t{
        ...
        strg->system_energy =
            units::energy::electron_volt_t{total_potential.value()} + 0.5 * defect_energy + 0.5 * defect_interaction;
    }
    /**
     ...
     */
    [[nodiscard]] units::energy::electron_volt_t get_system_energy() const noexcept
    {
    ...
    bool three_state_simulation_required() noexcept
    {
        ...
        this->foreach_cell(
            [&required, this](const auto& c)
            {
                if (const auto local_pot = this->get_local_potential(c); local_pot.has_value())
                {
                    if (-*local_pot + strg->phys_params.mu_p > -POP_STABILITY_ERR)
                    {
     ...
     */
    [[nodiscard]] units::voltage::volt_t
    chargeless_potential_at_given_distance(const units::length::nanometer_t& distance) const noexcept
    {
        if (distance == units::length::nanometer_t{0.0})
        {
            return 0.0_V;
        }
     ...
     */
    [[nodiscard]] units::voltage::volt_t
    chargeless_potential_generated_by_defect_at_given_distance(const units::length::nanometer_t& distance,
    ...
    void assign_local_external_potential(
        const std::unordered_map<typename Lyt::cell, units::voltage::volt_t>& external_voltage) noexcept
    {
     ...
     */
    std::unordered_map<typename Lyt::cell, units::voltage::volt_t> get_external_potentials() noexcept
    {
     ...
     */
    std::unordered_map<typename Lyt::cell, units::voltage::volt_t> get_defect_potentials() noexcept
    {
    ...
    void update_charge_state_of_dependent_cell() noexcept
    {
        if (!strg->dependent_cell.is_dead())
        {
            ...
            if (loc_pot_cell + strg->phys_params.mu < POP_STABILITY_ERR)
            {
            ...
            else if (loc_pot_cell + strg->phys_params.mu_p > -POP_STABILITY_ERR)
            {
  ...
  private:
    ...
    {
        strg->nm_dist_mat = std::vector<std::vector<units::length::nanometer_t>>(
            this->num_cells(),
            std::vector<units::length::nanometer_t>(this->num_cells(), units::length::nanometer_t(0)));
    ...
    {
        strg->pot_mat = std::vector<std::vector<units::voltage::volt_t>>(
            this->num_cells(), std::vector<units::voltage::volt_t>(this->num_cells(), units::voltage::volt_t(0.0)));

        for (uint64_t i = 0u; i < strg->sidb_order.size(); ++i)
        {
            for (uint64_t j = 0u; j < strg->sidb_order.size(); j++)
            {
                strg->pot_mat[i][j] = potential_between_sidbs_by_indices(i, j);
            }
        }
    }
};
