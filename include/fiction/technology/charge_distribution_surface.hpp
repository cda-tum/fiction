...
class charge_distribution_surface<Lyt, false> : public Lyt
{
  public:
     ...
     */
    charge_distribution_surface(const charge_distribution_surface<Lyt>& lyt) :
            Lyt(lyt),
    ...
    void validity_check() noexcept
    {
        ...
        // if population stability is fulfilled for all SiDBs, the "configuration stability" is checked next
        if (for_loop_counter > 0)
        {
            const auto hop_delta =
                [this](const uint64_t c1, const uint64_t c2)  // energy change when charge hops between two SiDBs
            {
                ...
                return strg->loc_pot[c1] * dn_i + strg->loc_pot[c2] * dn_j - strg->pot_mat[c1][c2];
            };
            ...
            for (uint64_t i = 0u; i < strg->loc_pot.size(); ++i)
            {
                ...
                for (uint64_t j = 0u; j < strg->loc_pot.size(); j++)
                {
                    ...
                    if (const auto e_del = hop_delta(i, j);
