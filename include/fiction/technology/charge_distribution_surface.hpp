  ...
  public:
    ...
    struct charge_distribution_storage
    {
      ...
      public:
        ...
        /*
         * Local electrostatic potential generated by charged SiDBs.
         */
     ...
     * // todo
     */
    void update_potential_due_to_predefined_negatively_charged_sidbs(const typename Lyt::cell& negative_sidb) noexcept
    {
        this->foreach_cell(
            [this, &negative_sidb](const auto& c1)
            {
                const auto dist = sidb_nm_distance<Lyt>(*this, c1, negative_sidb);
                const auto pot  = chargeless_potential_at_given_distance(dist);

                strg->local_int_pot[static_cast<uint64_t>(cell_to_index(c1))] += pot * (-1.0);
    ...
    void update_local_defect_potential() noexcept
    {
        ...
        for (const auto& [c1, defect1] : strg->defects)
        {
            ...
            for (auto i = 0; i < strg->sidb_order.size(); ++i)
            {
    ...
    // todo
    void add_internal_and_external_local_potential() noexcept
    {
     ...
     * @param history_mode `charge_distribution_history::NEGLECT` if the information (local electrostatic energy) of the
     * previous charge distribution is used to make the update more efficient, `charge_distribution_history::CONSIDER`
     * otherwise.  TODO (consider = no ext pot update)
     */
        ...
        const charge_distribution_history history_mode = charge_distribution_history::NEGLECT) noexcept
    {
        ...
        add_internal_and_external_local_potential();
    }
    /**
