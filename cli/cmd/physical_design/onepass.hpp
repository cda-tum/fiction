  ...
  public:
    ...
    explicit onepass_command(const environment::ptr& e) :
            command(e, "SAT-driven topology-based logic re-synthesis, i.e., one-pass synthesis. Uses "
                       ...
                       "layout's borders.")
    {
        add_option("--clk_scheme,-s", ps.scheme, "Clocking scheme to use {2DDWAVE[3|4], USE, RES, ESP, BANCS}", true);
  ...
  protected:
    ...
    void execute() override
    {
        ...
        if (this->is_set("network"))
        {
            ...
            {
                env->out() << fmt::format("[e] \"{}\" does not refer to a supported clocking scheme", ps.scheme)
                           << std::endl;
