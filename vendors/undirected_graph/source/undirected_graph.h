...
class undirected_graph
{
  ...
  public:
    ...
    //! Iterator for vertices in the graph. It behaves like a std::unordered_map iterator with it->first being the
    //! vertex id and it->second being the mapped vertex data.
    typedef typename vertex_container::iterator graph_vertex_iterator;
    //! Const iterator for vertices in the graph. It behaves like a std::unordered_map iterator with it->first being the
    //! vertex id and it->second being the mapped vertex data.
    typedef typename vertex_container::const_iterator graph_vertex_const_iterator;
    //! Iterator for edges in the graph. It behaves like a std::unordered_map iterator with it->first being the edge id
    //! and it->second being the mapped edge data.
    typedef typename edge_container::iterator graph_edge_iterator;
    //! Const iterator for edges in the graph. It behaves like a std::unordered_map iterator with it->first being the
    //! edge id and it->second being the mapped edge data.
    typedef typename edge_container::const_iterator graph_edge_const_iterator;
    //! Iterator for adjacencent vertices in the graph. It behaves like a std::forward_list iterator dereferencing to
    //! the id of the adjacent vertex.
    typedef typename adjacency_list::iterator graph_adjacency_iterator;
    //! Const iterator for adjacencent vertices in the graph. It behaves like a std::forward_list iterator dereferencing
    //! to the id of the adjacent vertex.
     ...
     * @return An iterator to the vertex, if a vertex with specified id is found, or undirected_graph::end_vertices
     * otherwise.
     */
    ...
    graph_adjacency_const_iterator begin_adjacent(const vertex_id_type& vertex) const
    {
        return adjacency.at(vertex).begin();
    }
    ...
    graph_adjacency_const_iterator end_adjacent(const vertex_id_type& vertex) const
    {
        return adjacency.at(vertex).end();
    }
     ...
     * @brief Insert vertex
     *
     * Inserts a new vertex to the graph, effectively increasing the container size by one. Multiple vertices with the
     * same value may exist in one graph but ids have to be unique.
     * @param vertex_id The id of the vertex.
     * @param vertex_data Value to be copied to the inserted vertex.
     * @return Returns a pair with an iterator to the inserted vertex and a bool indicating whether the vertex was newly
     * inserted or not.
     */
    std::pair<graph_vertex_iterator, bool> insert_vertex(const vertex_id_type&   vertex_id,
                                                         const vertex_data_type& vertex_data)
    {
     ...
     * @return Returns a pair with an iterator to the inserted edge and a bool indicating whether the edge was inserted
     * or not.
     */
    std::pair<graph_edge_iterator, bool> insert_edge(const vertex_id_type& vertex_a, const vertex_id_type& vertex_b,
