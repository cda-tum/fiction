...
class undirected_pair
{
  public:
    ...
    //! Returns a reference to the other data element if compare is the same as one of the elements in the undirected
    //! pair
    ...
    //! Returns a const reference to the other data element if compare is the same as one of the elements in the
    //! undirected pair
    ...
    inline bool operator<(const undirected_pair& rhs) const
    {
        if (smaller_element() < rhs.smaller_element())
            return true;
        if (rhs.smaller_element() < smaller_element())
            return false;
        if (bigger_element() < rhs.bigger_element())
            return true;
        return false;
    }
    inline bool operator>(const undirected_pair& rhs) const
