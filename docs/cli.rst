...
- Randomize skip tiles PI placement (``-j``): Randomize the number of skipped tiles for each PI placement. When enabled, each PI will use a random number of skipped tiles between skip_tiles_pi_placement-1 and skip_tiles_pi_placement (inclusive). When skip_tiles_pi_placement is 0, only 0 will be used. This can help explore different placement strategies and potentially find better layouts.
