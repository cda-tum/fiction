...
in C++20 using the [EPFL Logic Synthesis Libraries](https://github.com/lsils/lstools-showcase). Thereby, _fiction_ focuses on the
...
To be as generic as possible, _fiction_ is able to perform physical design tasks for FCN circuit layouts on data
...
The _fiction_ framework is academic software and aims at researchers and developers in the FCN domain who want to obtain
cell-accurate circuit layouts from logical specifications or who want to implement their own physical design algorithms.

For these use cases, _fiction_ provides a {ref}`header-only library <header-only>` that provides data types and algorithms
for recurring tasks, e.g., logic network and layout types on different abstraction levels, clocking schemes, gate libraries,
placement, routing, clocking, and verification algorithms, etc. Additionally, _fiction_ comes with an ABC-like
