...
{
    const auto skeleton =
        read_sqd_layout<sidb_cell_clk_lyt_siqad>("/Users/jandrewniok/CLionProjects/fiction_copy/fiction/experiments/"
    ...
    const auto truth_tables = std::vector<std::vector<tt>>{
     ...
     //   const auto truth_tables = std::vector<std::vector<tt>>{std::vector<tt>{create_nor_tt()}};
    ...
    for (const auto& truth : truth_tables)
    {
        ...
        for (auto i = 4u; i < 5; i++)
        {
            ...
            for (auto t = 0u; t < 10; t++)
            {
                ...
                const auto all_gate = design_sidb_gates<sidb_cell_clk_lyt_siqad, tt>(skeleton, truth, params, &stats);
                net_cost += stats.cost_of_gate;
                total_time += mockturtle::to_seconds(stats.time_total);
            }
            std::cout << fmt::format("average runtime: {}", total_time / 10) << std::endl;
            std::cout << fmt::format("net cost: {}", net_cost / 10) << std::endl;
        }
