...
// generated layouts that may contain positively charged SiDBs (`can_positive_charge_occur` returns `true`). The number
...
int main()  // NOLINT
{
    ...
    for (auto num_sidbs = 5u; num_sidbs < 20; num_sidbs++)
    {
        ...
        for (const auto& layout : random_layouts)
        {
            ...
            const auto gs = groundstate_from_simulation_results(exhaustive_results_layout);
