...
int main()  // NOLINT
{
    experiments::experiment<std::string, std::size_t, double, double, double, double, double, double> simulation_exp{
        "Benchmark",
        "Gate Name",
        "#Instances",
        "Exhaustive Runtime [s]",
        "QuickExact Runtime [s]",
        "QuickComplete Runtime [s]",
    ...
    const clustercomplete_params<siqad::coord_t> clustering_params{sim_params};
    ...
    for (const auto& [gate, truth_table] : gates)
    {
        for (const auto& file : std::filesystem::directory_iterator(fmt::format("{}{}", folder, gate)))
        {
            ...
            double      runtime_clustercomplete = 0;
            ...
            const auto quickcomplete_results_layout = clustercomplete(layout, clustering_params);
            ...
            runtime_clustercomplete += mockturtle::to_seconds(quickcomplete_results_layout.simulation_runtime);
            ...
            for (auto i = 0u; i < num_input_patterns; ++i, ++bii)
            {
                ...
                const auto quickcomplete_results = clustercomplete(*bii, clustering_params);
                ...
                runtime_clustercomplete += mockturtle::to_seconds(quickcomplete_results.simulation_runtime);
            ...
            simulation_exp(gate, instances, runtime_exhaustive, runtime_quickexact, runtime_clustercomplete,
                           quicksim_accuracy_mean, quicksim_single_runtime, tts_quicksim);
            simulation_exp.save();
