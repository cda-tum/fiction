...
#include "fiction/algorithms/simulation/sidb/exact_ground_state_simulation_real_valued.hpp"
...
int main()  // NOLINT
{
    ...
    static const std::array folders = {folder + "and/", folder + "nand/", folder + "xnor/", folder + "xor/",
                                       folder + "or/"};
    ...
    for (const auto& folder_gate : folders)
    {
        for (const auto& file : std::filesystem::directory_iterator(folder_gate))
        {
            ...
            const auto                       exgs_simulation_results =
                exhaustive_ground_state_simulation<sidb_cell_clk_lyt_siqad>(lyt, phys_params);

            // TTS of QuickSim
            const quicksim_params  quicksim_params{phys_params, 80, 0.7};
            time_to_solution_stats tts_stat{};
            time_to_solution(lyt, quicksim_params, time_to_solution_params{}, &tts_stat);

            // SMT solver
            const exact_ground_state_simulation_real_params                   exact_params{phys_params};
            exact_ground_state_simulation_real_stats<sidb_cell_clk_lyt_siqad> exact_stats{};
            exact_ground_state_simulation_real<sidb_cell_clk_lyt_siqad>(lyt, exact_params, &exact_stats);

            sidb_simulation_result<sidb_cell_clk_lyt_siqad> smt_simulation_results{};
            smt_simulation_results.algorithm_name       = "SMT";
            smt_simulation_results.simulation_runtime   = exact_stats.time_total;
            smt_simulation_results.charge_distributions = exact_stats.valid_lyts;

            groundstate_found = is_ground_state(smt_simulation_results,
                                                exgs_simulation_results);  // checks if SMT found the ground state
            ...
            simulation_exp(benchmark.string(), mockturtle::to_seconds(exgs_simulation_results.simulation_runtime),
            ...
            sum_sr_exgs += mockturtle::to_seconds(exgs_simulation_results.simulation_runtime);
