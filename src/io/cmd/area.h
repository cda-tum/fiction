...
#ifndef FICTION_AREA_H
#define FICTION_AREA_H
...
namespace alice
{
    ...
    public:
         ...
         */
        explicit area_command(const environment::ptr& env)
                :
                command(env,
                        "Prints the area usage (in nm²) of the current cell layout in store. Dimensions for each "
                        "cell use default values from QCADesigner (QCA) or NMLSim (iNML) if no value is provided.")
        {
            add_option("--width,-x", width,
                       "Cell width in nm");
            add_option("--height,-y", height,
                       "Cell height in nm");
            add_option("--hspace", hspace,
                       "Horizontal cell spacing in nm");
            add_option("--vspace", vspace,
                       "Vertical cell spacing in nm");
        }
    ...
    protected:
        ...
        void execute() override
        {
            ...
            if (!is_set("width"))
            {
                width = fcl->get_technology() == fcn::technology::QCA ? area_defaults::qca::width : area_defaults::inml::width;
            }
            if (!is_set("height"))
            {
                height = fcl->get_technology() == fcn::technology::QCA ? area_defaults::qca::height : area_defaults::inml::height;
            }
            if (!is_set("hspace"))
            {
                hspace = fcl->get_technology() == fcn::technology::QCA ? area_defaults::qca::hspace : area_defaults::inml::hspace;
            }
            if (!is_set("vspace"))
            {
                vspace = fcl->get_technology() == fcn::technology::QCA ? area_defaults::qca::vspace : area_defaults::inml::vspace;
            }
