...
function(check_z3_version z3_include z3_lib)
  try_run(
    ...
    CMAKE_FLAGS -DINCLUDE_DIRECTORIES:STRING=${z3_include} LINK_LIBRARIES
                ${z3_lib}
    COMPILE_OUTPUT_VARIABLE
      COMPILE_OUTPUT RUN_OUTPUT_STDOUT_VARIABLE RUN_OUTPUT
  ...
  if(NOT Z3_COMPILE_RESULT OR RUN_OUTPUT_STDERR)
    if(NOT Z3_COMPILE_RESULT)
      message(
        STATUS
          "Could not compile test program for Z3 version check. Compile output: "
          ${COMPILE_OUTPUT})
    else()
      message(
        STATUS "Could not run test program for Z3 version check. Run output: "
               ${RUN_OUTPUT} " RUN_OUTPUT_STDERR: " ${RUN_OUTPUT_STDERR})
    endif()
    if(z3_include AND EXISTS "${z3_include}/z3_version.h")
      ...
      string(REGEX REPLACE "^.*Z3_BUILD_NUMBER[\t ]+([0-9]*).*$" "\\1" Z3_BUILD
                           "${z3_version_str}")
    ...
    string(REGEX MATCH "(Z3 )?([0-9]+.[0-9]+.[0-9]+.[0-9]+)" Z3_VERSION_STRING
                 ${RUN_OUTPUT})
    set(z3_version_string ${CMAKE_MATCH_2})
  endif()

  find_package_check_version(${z3_version_string} suitable_version
  ...
  if(Python_FOUND)
    execute_process(
      COMMAND ${Python_EXECUTABLE} -c
              "import os, z3; print(os.path.dirname(z3.__file__))"
    ...
    if(Z3_PYTHON_ROOT)
      ...
      if(Z3_CXX_INCLUDE_DIRS AND Z3_LIBRARIES)
        message(
          STATUS "Found Z3 includes and libraries from Python installation.")
        message(VERBOSE "Z3_CXX_INCLUDE_DIRS: ${Z3_CXX_INCLUDE_DIRS}")
        message(VERBOSE "Z3_LIBRARIES: ${Z3_LIBRARIES}")
        check_z3_version(${Z3_CXX_INCLUDE_DIRS} ${Z3_LIBRARIES})
      endif()
...
if(NOT FOUND_SUITABLE_VERSION)
  if(Z3_CXX_INCLUDE_DIRS AND Z3_LIBRARIES)
    message(
      STATUS
        "Found include and library directories but could not find a suitable Z3 version"
    )
  endif()
