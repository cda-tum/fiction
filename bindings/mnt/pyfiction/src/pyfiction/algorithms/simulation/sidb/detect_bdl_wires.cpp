...
void detect_bdl_wires_impl(nanobind::module_& m, const std::string& lattice)
{
    ...
    py::class_<bdl_wire_t>(m, fmt::format("bdl_wire_{}", lattice).c_str(), DOC(fiction_bdl_wire))
        ...
        .def(py::init<std::vector<fiction::sidb::simulation::logic::bdl_pair<fiction::layouts::coords::offset>>>(),
             py::arg("p"), DOC(fiction_bdl_wire_bdl_wire_2))
...
void sidb::simulation::logic::detect_bdl_wires(nanobind::module_& m)
{
    ...
    py::enum_<fiction::sidb::simulation::logic::bdl_wire_selection>(m, "bdl_wire_selection",
                                                                    DOC(fiction_bdl_wire_selection))
        .value("ALL", fiction::sidb::simulation::logic::bdl_wire_selection::ALL, DOC(fiction_bdl_wire_selection_ALL))
        .value("INPUT", fiction::sidb::simulation::logic::bdl_wire_selection::INPUT,
               DOC(fiction_bdl_wire_selection_INPUT))
        .value("OUTPUT", fiction::sidb::simulation::logic::bdl_wire_selection::OUTPUT,
               DOC(fiction_bdl_wire_selection_OUTPUT))
        .export_values();  // Export enum values to Python namespace

    // Class for detect_bdl_wires_params
    py::class_<fiction::sidb::simulation::logic::detect_bdl_wires_params>(m, "detect_bdl_wires_params",
                                                                          DOC(fiction_detect_bdl_wires_params))
