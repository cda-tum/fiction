...
def test_initialization():
    ...
    assert charge_lyt.get_charge_state(offset_coordinate(0, 1)) == sidb_charge_state.NEGATIVE
    assert charge_lyt.get_charge_state((4, 1)) == sidb_charge_state.NEGATIVE
    assert charge_lyt.get_charge_state((6, 1)) == sidb_charge_state.NEGATIVE
    ...
    assert charge_lyt.get_charge_state((0, 1)) == sidb_charge_state.NEUTRAL
    assert charge_lyt.get_charge_state((4, 1)) == sidb_charge_state.NEGATIVE
    assert charge_lyt.get_charge_state((6, 1)) == sidb_charge_state.NEGATIVE
    charge_lyt.update_after_charge_change()
    assert not charge_lyt.is_physically_valid()
    assert charge_lyt.num_neutral_sidbs() == 1
    assert charge_lyt.num_negative_sidbs() == 2
    ...
    charge_lyt.assign_charge_state((6, 1), sidb_charge_state.NEGATIVE)
    charge_lyt.update_after_charge_change()
    assert charge_lyt.is_physically_valid()
...
def test_initialization_111_lattice():
    ...
    assert charge_lyt.get_charge_state((0, 1)) == sidb_charge_state.NEGATIVE
    assert charge_lyt.get_charge_state((1, 1)) == sidb_charge_state.NEGATIVE
    assert charge_lyt.get_charge_state((2, 1)) == sidb_charge_state.NEGATIVE

    assert charge_lyt.get_charge_index_and_base()[0] == 0
    ...
    assert charge_lyt.get_charge_state((0, 1)) == sidb_charge_state.NEUTRAL
    assert charge_lyt.get_charge_state((1, 1)) == sidb_charge_state.NEGATIVE
    assert charge_lyt.get_charge_state((2, 1)) == sidb_charge_state.NEUTRAL
    ...
    assert charge_lyt.num_positive_sidbs() == 1
