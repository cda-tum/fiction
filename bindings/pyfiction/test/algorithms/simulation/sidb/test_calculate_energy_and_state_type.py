...
class TestCalculateEnergyAndStateType(unittest.TestCase):
    ...
    def test_calculate_energy_and_state_type(self):
        ...
        layout_one.assign_cell_type((0, 0), sidb_technology.cell_type.NORMAL)
        layout_one.assign_cell_type((1, 2), sidb_technology.cell_type.NORMAL)
        layout_one.assign_cell_type((5, 2), sidb_technology.cell_type.NORMAL)

        charge_lyt_one = charge_distribution_surface(layout_one)
        charge_lyt_one.assign_charge_state((0, 0), sidb_charge_state.NEUTRAL)
        charge_lyt_one.assign_charge_state((1, 2), sidb_charge_state.NEGATIVE)
        charge_lyt_one.assign_charge_state((5, 2), sidb_charge_state.NEGATIVE)
        charge_lyt_one.update_after_charge_change()
        energy_one = charge_lyt_one.get_system_energy()

        layout_two = sidb_layout((10, 10))
        layout_two.assign_cell_type((0, 0), sidb_technology.cell_type.NORMAL)
        layout_two.assign_cell_type((1, 2), sidb_technology.cell_type.NORMAL)
        layout_two.assign_cell_type((5, 2), sidb_technology.cell_type.NORMAL)
