...
void gate_level_layout(pybind11::module& m, const std::string& topology)
{
    ...
    py::class_<GateLyt, LytBase>(m, fmt::format("{}_gate_layout", topology).c_str(), DOC(fiction_gate_level_layout))
        ...
        .def(
            "get_node",
            [](const GateLyt& layout, const py_coordinate_offset_ucoord& coordinate)
            { return layout.get_node(coordinate); },
        ...
        .def("move_node", &GateLyt::move_node, "n"_a, "t"_a,
             "new_children"_a = std::vector<py_coordinate_offset_ucoord>{}, DOC(fiction_gate_level_layout_move_node))
