...
TEST_CASE("Cell technology", "[cell-level-layout]")
{
    SECTION("QCA")
    {
        CHECK(qca::technology::is_empty_cell(qca::technology::cell_type::EMPTY));
        CHECK(qca::technology::is_normal_cell(qca::technology::cell_type::NORMAL));
        CHECK(qca::technology::is_input_cell(qca::technology::cell_type::INPUT));
        CHECK(qca::technology::is_output_cell(qca::technology::cell_type::OUTPUT));
        CHECK(qca::technology::is_const_0_cell(qca::technology::cell_type::CONST_0));
        CHECK(qca::technology::is_const_1_cell(qca::technology::cell_type::CONST_1));
        CHECK(qca::technology::is_constant_cell(qca::technology::cell_type::CONST_0));
        CHECK(qca::technology::is_constant_cell(qca::technology::cell_type::CONST_1));

        CHECK(qca::technology::is_normal_cell_mode(qca::technology::cell_mode::NORMAL));
        CHECK(qca::technology::is_rotated_cell_mode(qca::technology::cell_mode::ROTATED));
        CHECK(qca::technology::is_vertical_cell_mode(qca::technology::cell_mode::VERTICAL));
        CHECK(qca::technology::is_crossover_cell_mode(qca::technology::cell_mode::CROSSOVER));

        CHECK(tech_impl_name<qca::technology> == std::string{"QCA"});
    ...
    SECTION("iNML")
    {
        CHECK(inml::technology::is_empty_cell(inml::technology::cell_type::EMPTY));
        CHECK(inml::technology::is_normal_cell(inml::technology::cell_type::NORMAL));
        CHECK(inml::technology::is_input_cell(inml::technology::cell_type::INPUT));
        CHECK(inml::technology::is_output_cell(inml::technology::cell_type::OUTPUT));
        CHECK(inml::technology::is_slanted_edge_up_magnet(inml::technology::cell_type::SLANTED_EDGE_UP_MAGNET));
        CHECK(inml::technology::is_slanted_edge_down_magnet(inml::technology::cell_type::SLANTED_EDGE_DOWN_MAGNET));
        CHECK(inml::technology::is_slanted_edge_magnet(inml::technology::cell_type::SLANTED_EDGE_UP_MAGNET));
        CHECK(inml::technology::is_slanted_edge_magnet(inml::technology::cell_type::SLANTED_EDGE_DOWN_MAGNET));
        CHECK(inml::technology::is_inverter_magnet(inml::technology::cell_type::INVERTER_MAGNET));
        CHECK(inml::technology::is_crosswire_magnet(inml::technology::cell_type::CROSSWIRE_MAGNET));
        CHECK(inml::technology::is_fanout_coupler_magnet(inml::technology::cell_type::FANOUT_COUPLER_MAGNET));

        CHECK(inml::technology::is_normal_cell_mode(inml::technology::cell_mode{}));

        CHECK(tech_impl_name<inml::technology> == std::string{"iNML"});

        CHECK(has_inml_technology_v<inml_cell_clk_lyt>);
    }
    SECTION("SiDB")
    {
        CHECK(sidb::technology::is_empty_cell(sidb::technology::cell_type::EMPTY));
        CHECK(sidb::technology::is_normal_cell(sidb::technology::cell_type::NORMAL));
        CHECK(sidb::technology::is_input_cell(sidb::technology::cell_type::INPUT));
        CHECK(sidb::technology::is_output_cell(sidb::technology::cell_type::OUTPUT));
        CHECK(sidb::technology::is_logic_cell(sidb::technology::cell_type::LOGIC));

        CHECK(sidb::technology::is_normal_cell_mode(sidb::technology::cell_mode{}));

        CHECK(tech_impl_name<sidb::technology> == std::string{"SiDB"});
        ...
        lyt.assign_cell_type({2, 1}, sidb::technology::cell_type::INPUT);
        lyt.assign_cell_type({6, 2}, sidb::technology::cell_type::NORMAL);
        lyt.assign_cell_type({8, 3}, sidb::technology::cell_type::NORMAL);
        lyt.assign_cell_type({12, 4}, sidb::technology::cell_type::LOGIC);
        lyt.assign_cell_type({14, 5}, sidb::technology::cell_type::OUTPUT);

        const auto input_cells  = lyt.get_cells_by_type(sidb::technology::cell_type::INPUT);
        const auto output_cells = lyt.get_cells_by_type(sidb::technology::cell_type::OUTPUT);
        const auto logic_cells  = lyt.get_cells_by_type(sidb::technology::cell_type::LOGIC);

        CHECK(input_cells.size() == 2);
        CHECK(output_cells.size() == 1);
        CHECK(logic_cells.size() == 1);

        lyt.assign_cell_type({0, 0}, sidb::technology::cell_type::NORMAL);
        lyt.assign_cell_type({14, 5}, sidb::technology::cell_type::NORMAL);
        lyt.assign_cell_type({12, 4}, sidb::technology::cell_type::NORMAL);
        ...
        lyt.assign_cell_type({14, 5}, sidb::technology::cell_type::INPUT);
        CHECK(lyt.num_pis() == 2);
        CHECK(lyt.num_pos() == 0);
    }
}
...
TEST_CASE("Cell type assignment", "[cell-level-layout]")
{
    ...
    CHECK(layout.get_cell_type({2, 0}) == qca::technology::cell_type::CONST_0);
    CHECK(layout.get_cell_type({2, 4}) == qca::technology::cell_type::INPUT);
    CHECK(layout.get_cell_type({0, 2}) == qca::technology::cell_type::INPUT);
    CHECK(layout.get_cell_type({2, 1}) == qca::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({2, 2}) == qca::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({2, 3}) == qca::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({1, 2}) == qca::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({3, 2}) == qca::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({4, 2}) == qca::technology::cell_type::OUTPUT);
    ...
    layout.assign_cell_type({0, 2}, qca::technology::cell_type::EMPTY);
    layout.assign_cell_type({2, 0}, qca::technology::cell_type::EMPTY);
    layout.assign_cell_type({2, 1}, qca::technology::cell_type::EMPTY);
    layout.assign_cell_type({4, 2}, qca::technology::cell_type::EMPTY);
...
TEST_CASE("Cell mode assignment", "[cell-level-layout]")
{
    ...
    CHECK(layout.get_cell_mode({0, 2, 0}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({2, 0, 0}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({1, 2, 0}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({2, 2, 0}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({3, 2, 0}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({2, 4, 0}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({4, 2, 0}) == qca::technology::cell_mode::NORMAL);

    CHECK(layout.get_cell_mode({2, 1, 1}) == qca::technology::cell_mode::CROSSOVER);
    CHECK(layout.get_cell_mode({2, 2, 1}) == qca::technology::cell_mode::CROSSOVER);
    CHECK(layout.get_cell_mode({2, 3, 1}) == qca::technology::cell_mode::CROSSOVER);

    // remove cell modes by assigning the normal mode
    layout.assign_cell_mode({2, 1, 1}, qca::technology::cell_mode::NORMAL);
    layout.assign_cell_mode({2, 2, 1}, qca::technology::cell_mode::NORMAL);
    layout.assign_cell_mode({2, 3, 1}, qca::technology::cell_mode::NORMAL);

    CHECK(layout.get_cell_mode({2, 1, 1}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({2, 2, 1}) == qca::technology::cell_mode::NORMAL);
    CHECK(layout.get_cell_mode({2, 3, 1}) == qca::technology::cell_mode::NORMAL);
}
