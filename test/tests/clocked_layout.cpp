...
TEST_CASE("Iteration", "[clocked]")
{
    ...
    CHECK(layout.incoming_clocked_tiles<std::set<tile_based_layout::tile>>({0, 0}).empty());
    CHECK(layout.outgoing_clocked_tiles<std::set<tile_based_layout::tile>>({2, 2}).empty());

    auto s1 = layout.incoming_clocked_tiles<std::set<tile_based_layout::tile>>({1, 1});
    auto s2 = std::set<tile_based_layout::tile>{{{1, 0}, {0, 1}}};

    CHECK(s1 == s2);

    auto s3 = layout.outgoing_clocked_tiles<std::set<tile_based_layout::tile>>({1, 1});
