...
def test_read_write(mux21, tmp_path):
    ...
    shifted_layout = shifted_cartesian_gate_layout(offset_coordinate(3, 3, 1), "2DDWave", "Layout")
...
def test_fgl_rejects_unrepresentable_layers(tmp_path: Path, value: str) -> None:
    layout = cartesian_gate_layout(offset_coordinate(1, 0), "2DDWave", "wire")
...
def test_fgl_preserves_labels_and_synchronization(tmp_path: Path) -> None:
    layout = cartesian_gate_layout(offset_coordinate(2, 0), "2DDWave", "A & B < C")
    source = layout.create_pi("a&b", offset_coordinate(0, 0))
    wire = layout.create_buf(source, offset_coordinate(1, 0))
    layout.create_po(wire, "f<g", offset_coordinate(2, 0))
    layout.assign_synchronization_element(offset_coordinate(1, 0), 2)
    path = tmp_path / "sync.fgl"
    write_fgl_layout(layout, str(path))
    restored = read_cartesian_fgl_layout(str(path))
    assert get_name(restored) == "A & B < C"
    assert restored.num_se() == 1
    assert restored.get_synchronization_element(offset_coordinate(1, 0)) == 2
