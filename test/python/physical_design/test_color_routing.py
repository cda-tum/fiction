...
def test_routing(make_lyt):
    ...
    success = color_routing(
        lyt,
        [
            (offset_coordinate(0, 0), offset_coordinate(2, 2)),
            (offset_coordinate(0, 1), offset_coordinate(2, 2)),
            (offset_coordinate(2, 2), offset_coordinate(4, 4)),
        ],
    )
...
def test_crossings():
    lyt = cartesian_gate_layout(offset_coordinate(4, 2, 1), "2DDWave", "Layout")

    x1 = lyt.create_pi("x1", offset_coordinate(0, 1))
    x2 = lyt.create_pi("x2", offset_coordinate(3, 2))
    x3 = lyt.create_pi("x3", offset_coordinate(2, 0))

    buf1 = lyt.create_buf(x3, offset_coordinate(2, 1))
    lyt.create_buf(buf1, offset_coordinate(2, 2))

    lyt.create_and(x1, x2, offset_coordinate(4, 2))
    lyt.move_node(lyt.get_node(offset_coordinate(4, 2)), offset_coordinate(4, 2))
    ...
    success = color_routing(
        lyt,
        [(offset_coordinate(0, 1), offset_coordinate(4, 2)), (offset_coordinate(3, 2), offset_coordinate(4, 2))],
        params=params,
    )
