...
{
    CHECK(std::is_same_v<std::iterator_traits<
                             physical_design::utils::aspect_ratio_iterator<layouts::coords::offset>>::iterator_category,
                         std::forward_iterator_tag>);

    CHECK(std::is_same_v<
          std::iterator_traits<physical_design::utils::aspect_ratio_iterator<layouts::coords::offset>>::value_type,
          layouts::coords::offset>);
}

TEST_CASE("Aspect ratio iteration", "[aspect-ratio-iterator]")
{
    physical_design::utils::aspect_ratio_iterator<layouts::coords::offset> ari{1};
    ...
    for (auto i = 0; ari <= 4; ++ari, ++i)
    {
        switch (i)
        {
            case 0:
            {
                ...
                CHECK(ari == 1u);
                CHECK(ari == physical_design::utils::aspect_ratio_iterator<layouts::coords::offset>{
