...
TEST_CASE("SiQAD OR gate", "[is-operational]")
{
    ...
    {
        CHECK(is_operational(lat, std::vector<tt>{networks::utils::create_or_tt()}, op_params).first ==
    ...
    SECTION("determine if kinks induce layout to become non-operational")
    {
        const auto kink_induced_non_operational =
            is_kink_induced_non_operational(lat, std::vector<tt>{networks::utils::create_or_tt()}, op_params);
    ...
    {
        CHECK(
            is_operational(lat, std::vector<tt>{networks::utils::create_and_tt()}, op_params, input_wires, output_wires)
                .first == operational_status::NON_OPERATIONAL);
    }

    SECTION("determine if kinks induce layout to become non-operational")
    {
        CHECK(is_kink_induced_non_operational(lat, std::vector<tt>{networks::utils::create_or_tt()}, op_params,
...
TEST_CASE("Test is_physical_validity_feasible for empty canvas", "[is-operational]")
{
    ...
    const auto op_params =
        is_operational_params{.sim_params                = sidb::model::simulation_parameters{2, -0.32},
                              .sim_engine                = sidb_simulation_engine::QUICKEXACT,
    ...
    CHECK(is_operational(lyt, networks::utils::create_crossing_wire_tt(), op_params).first ==
          operational_status::NON_OPERATIONAL);
}
...
TEST_CASE("SiQAD NAND gate", "[is-operational]")
{
    ...
    {
        CHECK(is_operational(lat, std::vector<tt>{networks::utils::create_nand_tt()}, op_params).first ==
              operational_status::OPERATIONAL);
    }
    ...
    SECTION("use pre-determined I/O pins")
    {
        CHECK(is_operational(lat, std::vector<tt>{networks::utils::create_nand_tt()}, op_params, input_wires,
                             output_wires, std::optional{canvas_lyt})
...
TEST_CASE("SiQAD's AND gate with input BDL pairs of different size", "[is-operational]")
{
    ...
    CHECK(is_operational(lat, std::vector<tt>{networks::utils::create_and_tt()},
                         is_operational_params{sidb::model::simulation_parameters{2, -0.28}})
              .first == operational_status::OPERATIONAL);
    CHECK(is_operational(lat, std::vector<tt>{networks::utils::create_and_tt()},
                         is_operational_params{sidb::model::simulation_parameters{2, -0.1}})
              .first == operational_status::NON_OPERATIONAL);
}
...
TEST_CASE("Bestagon FO2 gate", "[is-operational]")
{
    ...
    {
        CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_fan_out_tt()},
                             is_operational_params{sidb::model::simulation_parameters{2, -0.32},
                                                   sidb_simulation_engine::QUICKEXACT})
                  .first == operational_status::OPERATIONAL);
        CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_fan_out_tt()},
                             is_operational_params{sidb::model::simulation_parameters{2, -0.30},
                                                   sidb_simulation_engine::QUICKEXACT})
                  .first == operational_status::NON_OPERATIONAL);
    }
    ...
    SECTION("using QuickSim")
    {
        CHECK(is_operational(
                  ...
                  is_operational_params{sidb::model::simulation_parameters{2, -0.32}, sidb_simulation_engine::QUICKSIM})
                  .first == operational_status::OPERATIONAL);
        CHECK(is_operational(
                  lyt, std::vector<tt>{networks::utils::create_fan_out_tt()},
                  is_operational_params{sidb::model::simulation_parameters{2, -0.30}, sidb_simulation_engine::QUICKSIM})
                  .first == operational_status::NON_OPERATIONAL);
    }
    ...
    {
        CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_fan_out_tt()},
                             is_operational_params{sidb::model::simulation_parameters{3, -0.32},
                                                   sidb_simulation_engine::CLUSTERCOMPLETE})
                  .first == operational_status::OPERATIONAL);
        CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_fan_out_tt()},
                             is_operational_params{sidb::model::simulation_parameters{3, -0.30},
                                                   sidb_simulation_engine::CLUSTERCOMPLETE})
                  .first == operational_status::NON_OPERATIONAL);
    }
...
TEST_CASE("Bestagon CROSSING gate", "[is-operational]")
{
    ...
    CHECK(is_operational(
              lat, networks::utils::create_crossing_wire_tt(),
              is_operational_params{sidb::model::simulation_parameters{2, -0.30}, sidb_simulation_engine::QUICKEXACT})
              .first == operational_status::NON_OPERATIONAL);
}
...
TEST_CASE("Bestagon AND gate", "[is-operational]")
{
    ...
    SECTION("Without defects")
    {
        ...
        CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_and_tt()},
                             is_operational_params{sidb::model::simulation_parameters{2, -0.32},
                                                   sidb_simulation_engine::QUICKEXACT})
                  .first == operational_status::OPERATIONAL);
        CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_and_tt()},
    ...
    SECTION("Count the number of non-operational input combinations, accepting kinks")
    {
        const auto op_inputs =
            operational_input_patterns(lyt, std::vector<tt>{networks::utils::create_and_tt()},
                                       is_operational_params{.sim_params = sidb::model::simulation_parameters{2, -0.30},
                                                             .sim_engine = sidb_simulation_engine::QUICKEXACT});
...
TEST_CASE("SiQAD AND gate", "[is-operational]")
{
    ...
    SECTION("check in regime with positive charges")
    {
        SECTION("base 2")
        {
            ...
            CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_and_tt()}, is_operational_params{params})
                      .first == operational_status::NON_OPERATIONAL);
            CHECK(operational_input_patterns(lyt, std::vector<tt>{networks::utils::create_and_tt()},
                   ...
                   cds_sidb_111_cell_clk_lyt_siqad)
{
            ...
            "pairs are located at the bottom, while the output BDL pairs are at the top.")
    {
        ...
        const auto op_inputs =
            operational_input_patterns(lyt_mirrored_x, std::vector<tt>{networks::utils::create_and_tt()},
                                       is_operational_params{.sim_params = sidb::model::simulation_parameters{2, -0.32},
                                                             .sim_engine = sidb_simulation_engine::QUICKEXACT});
        CHECK(op_inputs.size() == 4);
        CHECK(op_inputs == std::set<uint64_t>{0, 1, 2, 3});
...
TEST_CASE("BDL wire", "[is-operational]")
{
    ...
    CHECK(is_operational(lyt, std::vector<tt>{networks::utils::create_id_tt()}, params).first ==
          operational_status::OPERATIONAL);
}
...
TEST_CASE("flipped CX bestagon gate", "[is-operational]")
{
    ...
    CHECK(is_operational(lyt, networks::utils::create_crossing_wire_tt(),
                         is_operational_params{sidb::model::simulation_parameters{2, -0.32},
                                               sidb_simulation_engine::QUICKEXACT, bdl_input_iterator_params{},
                                               is_operational_params::operational_condition::REJECT_KINKS})
              .first == operational_status::OPERATIONAL);
...
TEST_CASE("is operational check for Bestagon CX gate", "[is-operational], [quality]")
{
    ...
    {
        CHECK(is_operational(lat, networks::utils::create_crossing_wire_tt(),
                             is_operational_params{sidb::model::simulation_parameters{2, -0.32},
                                                   sidb_simulation_engine::QUICKEXACT})
                  .first == operational_status::OPERATIONAL);
        CHECK(is_operational(lat, networks::utils::create_crossing_wire_tt(),
    ...
    SECTION("using predetermined wires and only applying pruning without simulation")
    {
        ...
        auto op_params = is_operational_params{.sim_params = sidb::model::simulation_parameters{2, -0.32}};
        op_params.strategy_to_analyze_operational_status =
            is_operational_params::operational_analysis_strategy::FILTER_ONLY;

        CHECK(is_operational(lat, networks::utils::create_crossing_wire_tt(), op_params, input_bdl_wires,
                             output_bdl_wires)
                  .first == operational_status::OPERATIONAL);
    }
}
...
TEST_CASE("is operational check for Bestagon half adder", "[is-operational], [quality]")
{
    ...
    CHECK(is_operational(
              lat, networks::utils::create_half_adder_tt(),
              is_operational_params{sidb::model::simulation_parameters{2, -0.25}, sidb_simulation_engine::QUICKEXACT})
              .first == operational_status::NON_OPERATIONAL);
}
#endif
...
TEST_CASE("Pre-generated input pattern layouts match the layout-based overload", "[is-operational]")
{
    ...
    SECTION("SiQAD AND gate")
    {
                                     ...
                                     is_operational_params::operational_condition::REJECT_KINKS})
        {
            ...
            {
                is_operational_params params{.sim_params = sidb::model::simulation_parameters{2, mu_minus},
                                             .sim_engine = sidb_simulation_engine::QUICKEXACT};
    ...
    SECTION("SiQAD OR gate")
    {
        ...
        is_operational_params params{.sim_params = sidb::model::simulation_parameters{2, -0.28},
                                     .sim_engine = sidb_simulation_engine::QUICKEXACT};
    ...
    SECTION("Bestagon AND gate")
    {
        ...
        const is_operational_params params{.sim_params = sidb::model::simulation_parameters{2, -0.32},
                                           .sim_engine = sidb_simulation_engine::QUICKEXACT};
...
TEST_CASE("Pre-generated input pattern layouts are validated", "[is-operational]")
{
    ...
    SECTION("Too few layouts for the specification")
    {
        ...
        CHECK_THROWS_AS(is_operational(too_few, std::vector<tt>{networks::utils::create_and_tt()}, params, input_wires,
                                       output_wires),
...
TEST_CASE("Both is_operational entry points apply the same canvas rule", "[is-operational]")
{
                                 ...
                                 is_operational_params::operational_condition::REJECT_KINKS})
    {
        for (const auto strategy : {is_operational_params::operational_analysis_strategy::SIMULATION_ONLY,
                                    ...
                                    is_operational_params::operational_analysis_strategy::FILTER_THEN_SIMULATION})
        {
            is_operational_params params{.sim_params = sidb::model::simulation_parameters{2}};
            params.op_condition                           = condition;
