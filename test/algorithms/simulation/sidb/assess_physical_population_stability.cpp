...
TEST_CASE("Bestagon AND gate", "[assess-physical-population-stability]")
{
    ...
    SECTION("input 00")
    {
        ...
        CHECK(population_stability_detail.minimum_potential_difference_to_transition.at(
                  transition_type::NEUTRAL_TO_NEGATIVE) < 0.026);
        REQUIRE_THAT(
            population_stability_detail.distance_corresponding_to_potential.at(transition_type::NEUTRAL_TO_NEGATIVE),
    ...
    SECTION("input 01")
    {
        ...
        CHECK(population_stability_detail.minimum_potential_difference_to_transition.at(
                  transition_type::NEUTRAL_TO_NEGATIVE) < 0.041);
        REQUIRE_THAT(
            population_stability_detail.distance_corresponding_to_potential.at(transition_type::NEUTRAL_TO_NEGATIVE),
    ...
    SECTION("input 10")
    {
        ...
        CHECK(population_stability_detail.minimum_potential_difference_to_transition.at(
                  transition_type::NEUTRAL_TO_NEGATIVE) < 0.02);
        REQUIRE_THAT(
            population_stability_detail.distance_corresponding_to_potential.at(transition_type::NEUTRAL_TO_NEGATIVE),
...
{
    const auto params = assess_physical_population_stability_params{};
    auto       lyt    = blueprints::bestagon_crossing_gate<layout>();

    lyt.assign_cell_type({0, 0, 0}, layout::technology::cell_type::EMPTY);
    lyt.assign_cell_type({38, 0, 0}, layout::technology::cell_type::EMPTY);
    ...
    CHECK(population_stability_detail.minimum_potential_difference_to_transition.at(
              transition_type::NEUTRAL_TO_NEGATIVE) < 0.01);
    REQUIRE_THAT(
        population_stability_detail.distance_corresponding_to_potential.at(transition_type::NEUTRAL_TO_NEGATIVE),
...
TEST_CASE("Bestagon CROSSING gate input 11, using cube coordinates", "[assess-physical-population-stability]")
{
    ...
    const auto lyt_cube = convert_to_fiction_coordinates<sidb_100_cell_clk_lyt_cube>(lyt);

    CHECK(lyt.num_cells() == 27);

    const auto result = assess_physical_population_stability(lyt_cube, params);
    REQUIRE(result.size() == 20);
    const auto& population_stability_detail = result[0];
...
TEST_CASE("Bestagon CROSSING gate input 11, using offset coordinates", "[assess-physical-population-stability]")
{
    ...
    const auto lyt_offset = convert_to_fiction_coordinates<sidb_100_cell_clk_lyt>(lyt);

    CHECK(lyt.num_cells() == 27);

    const auto result = assess_physical_population_stability(lyt_offset, params);
    REQUIRE(result.size() == 20);
    const auto& population_stability_detail = result[0];
