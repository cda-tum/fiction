...
TEST_CASE("Binomial Coefficient Tests")
{
    SECTION("C(5, 2)")
    {
        uint64_t result = binomial_coefficient(5, 2);
        REQUIRE(result == 10);  // C(5, 2) = 10
    }

    SECTION("C(10, 3)")
    {
        uint64_t result = binomial_coefficient(10, 3);
        REQUIRE(result == 120);  // C(10, 3) = 120
    }

    SECTION("C(0, 0)")
    {
        uint64_t result = binomial_coefficient(0, 0);
        REQUIRE(result == 1);  // C(0, 0) = 1
    }

    SECTION("C(8, 8)")
    {
        uint64_t result = binomial_coefficient(8, 8);
        REQUIRE(result == 1);  // C(8, 8) = 1
    }

    SECTION("C(7, 10)")
    {
        uint64_t result = binomial_coefficient(7, 10);
        REQUIRE(result == 0);  // C(7, 10) = 0 (k > n)
    }
    SECTION("C(15, 7)")
    {
        uint64_t result = binomial_coefficient(15, 7);
        REQUIRE(result == 6435);  // C(15, 7) = 6435
    }

    SECTION("C(20, 10)")
    {
        uint64_t result = binomial_coefficient(20, 10);
        REQUIRE(result == 184756);  // C(20, 10) = 184,756
    }

    SECTION("C(30, 15)")
    {
        uint64_t result = binomial_coefficient(30, 15);
        REQUIRE(result == 155117520);  // C(30, 15) = 155,117,520
    }

    SECTION("C(50, 25)")
    {
        uint64_t result = binomial_coefficient(50, 25);
...
TEST_CASE("Generate Combinations Test")
{
    ...
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(3, 5);
        REQUIRE(result.size() == 10);  // There should be 10 combinations.
    }

    SECTION("Test case 2: Generating combinations of 2 elements from 4.")
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(2, 4);
    ...
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(1, 3);
    ...
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(0, 2);
        REQUIRE(result.size() == 1);  // There should be 1 combination (empty set).
        CHECK(result.front().size() == 0);
    }

    SECTION("Generating combinations of 5 elements from 5.")
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(5, 5);
    ...
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(0, 0);
        REQUIRE(result.size() == 1);  // There should be 1 combination (empty set).
        CHECK(result.front().size() == 0);
    }
    ...
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(3, 20);
    ...
    {
        std::vector<std::vector<uint64_t>> result = generate_unsigned_integer_combinations(3, 200);
