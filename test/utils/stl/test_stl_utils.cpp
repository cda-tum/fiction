...
TEST_CASE("Test find_first_two_of with vector input", "[find_first_two_of]")
{
    ...
    auto it = fiction::utils::stl::find_first_two_of(v1.begin(), v1.end(), v2.begin(), v2.end());
    CHECK(*it == 1);
    CHECK(*(it + 1) == 2);

    static const std::vector<int> v3{1, 2, 4, 3};
    it = fiction::utils::stl::find_first_two_of(v1.begin(), v1.end(), v3.begin(), v3.end());
...
TEST_CASE("Test find_first_two_of with array input", "[find_first_two_of]")
{
    ...
    auto it = fiction::utils::stl::find_first_two_of(std::begin(a1), std::end(a1), std::begin(a2), std::end(a2));
    CHECK(*it == 1);
    CHECK(*(std::next(it, 1)) == 2);

    static constexpr const std::array a3{1, 2, 4, 3};
    it = fiction::utils::stl::find_first_two_of(std::begin(a1), std::end(a1), std::begin(a3), std::end(a3));
...
TEST_CASE("Test find_first_two_of with different iterator types", "[find_first_two_of]")
{
    ...
    auto it = fiction::utils::stl::find_first_two_of(v1.begin(), v1.end(), std::begin(a2), std::end(a2));
    CHECK(*it == 1);
    CHECK(*(it + 1) == 2);

    static constexpr const std::array a3{1, 2, 4, 3};
    it = fiction::utils::stl::find_first_two_of(v1.begin(), v1.end(), std::begin(a3), std::end(a3));
    CHECK(it == std::next(v1.begin(), 2));
}
...
TEST_CASE("Test find_first_two_of with layout_coordinate_paths", "[first_first_two_of]")
{
    ...
    const auto it2 = fiction::utils::stl::find_first_two_of(p2.begin(), p2.end(), p1.begin(), p1.end());
    ...
    const auto it3 =
        fiction::utils::stl::find_first_two_of(std::cbegin(p1), std::cend(p1), std::cbegin(p2), std::cend(p2));
    const auto it4 =
        fiction::utils::stl::find_first_two_of(std::cbegin(p2), std::cend(p2), std::cbegin(p1), std::cend(p1));
