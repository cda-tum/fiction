...
TEST_CASE("SiDB position in nanometer for siqad coordinates", "[sidb-nm-position]")
{
    ...
    SECTION("Default lattice constants, positive cell coordinates")
    {
        ...
        CHECK_THAT(pos3_y, WithinAbs(params.lat_b * 0.8 + params.lat_c.second * 0.1, 1E-5));
        ...
        CHECK_THAT(pos5_y, WithinAbs(params.lat_b * 0.1 + params.lat_c.second * 0.1, 1E-5));
        ...
        CHECK_THAT(pos6_y, WithinAbs(params.lat_b + params.lat_c.second * 0.1, 1E-5));
    }
    ...
    SECTION("Default (Si-100) lattice constants, negative cell coordinates")
    {
        ...
        const auto [pos4_x, pos4_y] = sidb_nm_position<sidb_cell_clk_lyt_siqad>(params, {1, 1, 0});
    ...
    {
        const sidb_simulation_parameters params{2, 0, 0, 0, lattice_orientation::SI_111};

        const auto [pos0_x, pos0_y] = sidb_nm_position<sidb_cell_clk_lyt_siqad>(params, {0, 0, 0});
        CHECK_THAT(pos0_x, WithinAbs(0, 1E-5));
        CHECK_THAT(pos0_y, WithinAbs(0, 1E-5));

        const auto [pos_x, pos_y] = sidb_nm_position<sidb_cell_clk_lyt_siqad>(params, {0, 0, 1});
        ...
        const auto [pos4_x, pos4_y] = sidb_nm_position<sidb_cell_clk_lyt_siqad>(params, {-1, 0, 0});
...
TEST_CASE("SiDB position in nanometer for fiction coordinates", "[sidb-nm-position]")
{
    ...
    SECTION("Default lattice constants, negative cell coordinates")
    {
        ...
        CHECK_THAT(pos4_x, WithinAbs(-params.lat_a * 0.1, 1E-5));
        CHECK_THAT(pos4_y, WithinAbs(-params.lat_b * 0.1 + params.lat_c.second * 0.1, 1E-5));

        const auto [pos5_x, pos5_y] = sidb_nm_position<sidb_cell_clk_lyt_cube>(params, {-1, -21});
        CHECK_THAT(pos5_x, WithinAbs(-params.lat_a * 0.1, 1E-5));
        CHECK_THAT(pos5_y, WithinAbs(-params.lat_b + params.lat_c.second * 0.1, 1E-5));
    }
}
