...
static void
check_op_domain_params_and_operational_status(const OpDomain&                                           op_domain,
                                              const sidb::simulation::logic::operational_domain_params& params,
                                              const std::optional<sidb::simulation::logic::operational_status>& status)
{
    ...
    op_domain.for_each(
                           ...
                           const auto& op_value)  // NOLINT(misc-unused-parameters)
        {
            ...
            if (status.has_value())
            {
                if (status.value() == sidb::simulation::logic::operational_status::OPERATIONAL)
                {
...
TEST_CASE("Test parameter point", "[operational-domain]")
{
    ...
    const sidb::simulation::logic::parameter_point p_default;
    REQUIRE(p_default.get_parameters().empty());

    // Test parameterized constructor
    const std::vector<double>                      values = {1.0, 2.0, 3.0};
    const sidb::simulation::logic::parameter_point p_param(values);
    REQUIRE(p_param.get_parameters() == values);

    // Test equality operator
    const sidb::simulation::logic::parameter_point p1({1.0, 2.0, 3.0});
    const sidb::simulation::logic::parameter_point p2({1.0, 2.0, 3.0});
    const sidb::simulation::logic::parameter_point p3({1.0, 2.0, 3.0000001});
    ...
    const sidb::simulation::logic::parameter_point p4({1.0, 2.0, 3.1});
    REQUIRE(p1 != p4);
    ...
    SECTION("Equal parameter points hash equally")
    {
        ...
        const std::hash<sidb::simulation::logic::parameter_point> hash{};
    ...
    SECTION("Negative parameter values are hashable")
    {
        ...
        // undefined behavior
        const sidb::simulation::logic::parameter_point negative({-0.32, -0.5, -1.0});

        const std::hash<sidb::simulation::logic::parameter_point> hash{};

        CHECK(hash(negative) == hash(sidb::simulation::logic::parameter_point{{-0.32, -0.5, -1.0}}));
        CHECK(hash(negative) != hash(sidb::simulation::logic::parameter_point{{0.32, 0.5, 1.0}}));

        // and they must survive a round trip through a hash-based container
        const std::unordered_set<sidb::simulation::logic::parameter_point> points{
            negative, sidb::simulation::logic::parameter_point{{-0.28, -0.5, -1.0}}};
...
TEST_CASE("operational_domain class member functions", "[operational-domain]")
{
    ...
    CHECK(opdom.get_dimension(0) == sidb::simulation::logic::sweep_parameter::EPSILON_R);
    REQUIRE_THROWS_AS(opdom.get_dimension(1), std::out_of_range);

    opdom = sidb::simulation::logic::operational_domain(
        {sidb::simulation::logic::sweep_parameter::LAMBDA_TF, sidb::simulation::logic::sweep_parameter::MU_MINUS});
    CHECK(opdom.get_number_of_dimensions() == 2);
    CHECK(opdom.get_dimension(0) == sidb::simulation::logic::sweep_parameter::LAMBDA_TF);
    CHECK(opdom.get_dimension(1) == sidb::simulation::logic::sweep_parameter::MU_MINUS);
}
...
TEST_CASE("Error handling of operational domain algorithms", "[operational-domain]")
{
    ...
    {
        sidb::simulation::logic::operational_domain_params zero_dimensional_params{};
        sidb::simulation::logic::operational_domain_params one_dimensional_params{};

        // 0-dimensional
        zero_dimensional_params.sweep_dimensions = {};

        // 1-dimensional
        one_dimensional_params.sweep_dimensions = {{.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R}};
    ...
    SECTION("operational domain sketch preconditions")
    {
        ...
        REQUIRE(and_lat.num_cells_of_given_type(sidb::sidb_technology::cell_type::LOGIC) > 0);

        sidb::simulation::logic::operational_domain_params sketch_params{};
        ...
        sketch_params.operational_params.op_condition =
            sidb::simulation::logic::is_operational_params::operational_condition::REJECT_KINKS;
        sketch_params.sweep_dimensions = {
            {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.5, .max = 5.6, .step = 0.1},
            {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 5.0, .max = 5.1, .step = 0.1}};
    ...
    {
        sidb::simulation::logic::operational_domain_params invalid_params_1{};
        sidb::simulation::logic::operational_domain_params invalid_params_2{};
        sidb::simulation::logic::operational_domain_params invalid_params_3{};

        SECTION("min/max mismatch")
        {
            // 1-dimensional with invalid min/max on 1st dimension
            invalid_params_1.sweep_dimensions = {{.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R}};
            ...
            invalid_params_2.sweep_dimensions = {{.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R},
                                                 {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF}};
            ...
            invalid_params_3.sweep_dimensions = {{.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R},
                                                 {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF},
                                                 {.dimension = sidb::simulation::logic::sweep_parameter::MU_MINUS}};
...
TEST_CASE("SiQAD OR gate", "[operational-domain]")
{
    ...
    op_domain_params.sweep_dimensions = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 7, .max = 8, .step = 0.01},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 5.5, .max = 6, .step = 0.01}};
...
TEST_CASE("Three-dimensional operational domain sketch", "[operational-domain]")
{
    ...
    sidb::simulation::logic::operational_domain_params params{};
    ...
    params.sweep_dimensions = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.5, .max = 5.7, .step = 0.1},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 5.0, .max = 5.2, .step = 0.1},
    ...
    SECTION("grid search yields a superset of the simulated operational domain")
    {
        ...
        simulated.for_each(
            ...
            {
                if (std::get<0>(status) == sidb::simulation::logic::operational_status::OPERATIONAL)
                {
                    const auto sketched = sketch.contains(parameter_point);

                    REQUIRE(sketched.has_value());
                    CHECK(std::get<0>(sketched.value()) == sidb::simulation::logic::operational_status::OPERATIONAL);
                }
            });
    }
    ...
    SECTION("random sampling agrees with a direct operational check")
    {
        ...
        const auto input_wires = sidb::simulation::logic::detect_bdl_wires(
            lat, sketch_params.operational_params.input_bdl_iterator_params.bdl_wire_params,
...
TEST_CASE("Three-dimensional contour tracing", "[operational-domain]")
{
    ...
    params.operational_params.sim_engine = sidb::simulation::engine::QUICKEXACT;
    params.sweep_dimensions              = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.4, .max = 5.8, .step = 0.1},
...
TEST_CASE("Sampling zero points does not divide by zero", "[operational-domain]")
{
    ...
    sidb::simulation::logic::operational_domain_params op_domain_params{};
    op_domain_params.operational_params.sim_params = sidb::model::simulation_parameters{2, -0.32};
...
TEST_CASE("BDL wire operational domain computation", "[operational-domain]")
{
    ...
    op_domain_params.sweep_dimensions = {{.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R},
                                         {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF}};

    CHECK(op_domain_params.sweep_dimensions[0].dimension == sidb::simulation::logic::sweep_parameter::EPSILON_R);
    CHECK(op_domain_params.sweep_dimensions[1].dimension == sidb::simulation::logic::sweep_parameter::LAMBDA_TF);
    ...
    SECTION("operational area, only one parameter point")
    {
        ...
        SECTION("grid_search")
        {
            ...
            {
                constexpr auto z_dimension = sidb::simulation::logic::operational_domain_value_range{
                    .dimension = sidb::simulation::logic::sweep_parameter::MU_MINUS,
                    .min       = -0.32,
                    .max       = -0.32,
                    .step      = 0.01};
        ...
        SECTION("flood_fill")
        {
            ...
            SECTION("3-dimensional")
            {
                ...
                SECTION("one random sample")
                {
                    ...
                    // for the selected range, all samples should be within the parameters and operational
                    check_op_domain_params_and_operational_status(
                        op_domain_3d, op_domain_params, sidb::simulation::logic::operational_status::OPERATIONAL);
    ...
    SECTION("non-operational area")
    {
        ...
        SECTION("grid_search")
        {
            ...
            SECTION("3-dimensional")
            {
                ...
                // for the selected range, all samples should be within the parameters and non-operational
                check_op_domain_params_and_operational_status(
                    op_domain_3d, op_domain_params, sidb::simulation::logic::operational_status::NON_OPERATIONAL);
...
TEST_CASE("Contour tracing does not retrace an already enclosed area", "[operational-domain]")
{
    ...
    op_domain_params.sweep_dimensions = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 0.5, .max = 4.25, .step = 0.25},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 0.5, .max = 4.25, .step = 0.25}};
    ...
    for (auto i = 0; i < 5; ++i)
    {
        ...
        for (const auto& pp : inferred_points)
        {
            ...
            {
                CHECK(std::get<0>(*ground_truth) == sidb::simulation::logic::operational_status::OPERATIONAL);
            }
        }
...
TEST_CASE("Parallel flood fill yields deterministic results", "[operational-domain]")
{
    ...
    grid_search_domain.for_each(
        ...
        {
            if (std::get<0>(op_value) == sidb::simulation::logic::operational_status::OPERATIONAL)
            {
    ...
    std::ranges::sort(operational_points, std::ranges::less{},
                      &sidb::simulation::logic::parameter_point::get_parameters);
    ...
    std::optional<
        std::vector<std::pair<sidb::simulation::logic::parameter_point, sidb::simulation::logic::operational_status>>>
        reference{};
...
TEST_CASE("SiQAD's AND gate operational domain computation", "[operational-domain]")
{
    ...
    op_domain_params.operational_params.sim_params = sim_params;
    op_domain_params.sweep_dimensions              = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.1, .max = 6.0, .step = 0.1},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 4.5, .max = 5.4, .step = 0.1}};
...
TEST_CASE("SiQAD's AND gate operational domain computation, using cube coordinates", "[operational-domain]")
{
    ...
    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{0, 0, 1}),
                         sidb::sidb_technology::cell_type::INPUT);
    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{2, 1, 1}),
                         sidb::sidb_technology::cell_type::INPUT);

    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{20, 0, 1}),
                         sidb::sidb_technology::cell_type::INPUT);
    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{18, 1, 1}),
                         sidb::sidb_technology::cell_type::INPUT);

    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{4, 2, 1}),
                         sidb::sidb_technology::cell_type::NORMAL);
    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{6, 3, 1}),
                         sidb::sidb_technology::cell_type::NORMAL);

    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{14, 3, 1}),
                         sidb::sidb_technology::cell_type::NORMAL);
    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{16, 2, 1}),
                         sidb::sidb_technology::cell_type::NORMAL);

    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{10, 6, 0}),
                         sidb::sidb_technology::cell_type::OUTPUT);
    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{10, 7, 0}),
                         sidb::sidb_technology::cell_type::OUTPUT);

    lyt.assign_cell_type(layouts::coords::from_siqad<layouts::coords::cube>(layouts::coords::siqad{10, 9, 1}),
                         sidb::sidb_technology::cell_type::NORMAL);
...
TEMPLATE_TEST_CASE("AND gate on the H-Si(111)-1x1 surface", "[operational-domain]", sidb_111_cell_clk_lyt_siqad)
{
    ...
    op_domain_params.operational_params.sim_params = sim_params;
    op_domain_params.sweep_dimensions              = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.60, .max = 5.61, .step = 0.01},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 5.0, .max = 5.01, .step = 0.01}};
                   ...
                   "[operational-domain]", sidb_100_cell_clk_lyt_siqad)
{
    ...
    op_domain_params.operational_params.sim_params = sim_params;
    op_domain_params.sweep_dimensions              = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 4.0, .max = 6.0, .step = 0.4},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 4.0, .max = 6.0, .step = 0.4}};
...
{
    sidb::simulation::logic::critical_temperature_domain ctdom{};
    ...
    ctdom.add_dimension(sidb::simulation::logic::sweep_parameter::EPSILON_R);
    CHECK(ctdom.get_number_of_dimensions() == 1);
    CHECK(ctdom.get_dimension(0) == sidb::simulation::logic::sweep_parameter::EPSILON_R);
    REQUIRE_THROWS_AS(ctdom.get_dimension(1), std::out_of_range);

    ctdom = sidb::simulation::logic::critical_temperature_domain(
        {sidb::simulation::logic::sweep_parameter::LAMBDA_TF, sidb::simulation::logic::sweep_parameter::MU_MINUS});
    CHECK(ctdom.get_number_of_dimensions() == 2);
    CHECK(ctdom.get_dimension(0) == sidb::simulation::logic::sweep_parameter::LAMBDA_TF);
    CHECK(ctdom.get_dimension(1) == sidb::simulation::logic::sweep_parameter::MU_MINUS);

    ctdom = sidb::simulation::logic::critical_temperature_domain({sidb::simulation::logic::sweep_parameter::LAMBDA_TF,
                                                                  sidb::simulation::logic::sweep_parameter::EPSILON_R,
                                                                  sidb::simulation::logic::sweep_parameter::MU_MINUS});
    CHECK(ctdom.get_number_of_dimensions() == 3);
    CHECK(ctdom.get_dimension(0) == sidb::simulation::logic::sweep_parameter::LAMBDA_TF);
    CHECK(ctdom.get_dimension(1) == sidb::simulation::logic::sweep_parameter::EPSILON_R);
    CHECK(ctdom.get_dimension(2) == sidb::simulation::logic::sweep_parameter::MU_MINUS);
}
          ...
          "[operational-domain]")
{
    ...
    op_domain_params.operational_params.sim_params = sim_params;
    op_domain_params.sweep_dimensions              = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.6, .max = 5.8, .step = 0.1},
    ...
    {
        op_domain_params.sweep_dimensions = {
            {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 5.0, .max = 5.2, .step = 0.1},
            {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 4.9, .max = 5.1, .step = 0.1}};
...
TEST_CASE("Two BDL pair wire with degeneracy for input 1", "[operational-domain]")
{
    ...
    op_domain_params.operational_params.sim_params = sim_params;
    op_domain_params.sweep_dimensions              = {
        {.dimension = sidb::simulation::logic::sweep_parameter::EPSILON_R, .min = 1, .max = 10, .step = 0.1},
        {.dimension = sidb::simulation::logic::sweep_parameter::LAMBDA_TF, .min = 1, .max = 10, .step = 0.1}};
