...
TEMPLATE_TEST_CASE("Assign and delete charge states without defects", "[charge-distribution-surface]",
                   ...
                   (lattice<lattice_100, defect_surface<sidb_cell_clk_lyt_siqad>>))
{
    TestType lyt{};
    ...
    SECTION("cell to index and vs")
    {
        ...
        const charge_distribution_surface charge_layout{lyt, simulation_parameters{}};
        ...
        CHECK(charge_layout.index_to_cell(4) == (coords::siqad()));
        uint64_t found = 0;
        for (uint64_t i = 0u; i < 3; i++)
        {
            if (charge_layout.index_to_cell(i) == (coords::siqad(5, 4)))
            {
                found += 1;
            }
        }
        ...
        for (uint64_t i = 0u; i < 3; i++)
        {
            if (charge_layout.index_to_cell(i) == (coords::siqad(5, 5)))
            {
                found += 1;
            }
        }
        ...
        for (uint64_t i = 0u; i < 3; i++)
        {
            if (charge_layout.index_to_cell(i) == (coords::siqad(5, 6)))
            {
                found += 1;
            }
        }
    ...
    SECTION("charge distribution defined by a given charge index (base 3)")
    {
        ...
        lyt.assign_cell_type({5, 5}, TestType::cell_type::NORMAL);
        lyt.assign_cell_type({5, 6}, TestType::cell_type::NORMAL);
        charge_distribution_surface charge_layout{lattice{lyt}, simulation_parameters{}};
        charge_layout.assign_charge_index(charge_layout.get_max_charge_index());
        CHECK(charge_layout.get_charge_state({5, 4}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 6}) == charge_state::POSITIVE);

        charge_layout.assign_charge_index(0);
        CHECK(charge_layout.get_charge_state({5, 4}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 6}) == charge_state::NEGATIVE);
    }
    ...
    SECTION("charge distribution defined by a given charge index (base 2)")
    {
        ...
        lyt.assign_cell_type({5, 5}, TestType::cell_type::NORMAL);
        lyt.assign_cell_type({5, 6}, TestType::cell_type::NORMAL);
        charge_distribution_surface charge_layout{lyt, simulation_parameters{2}};
        charge_layout.assign_charge_index(charge_layout.get_max_charge_index());
        CHECK(charge_layout.get_charge_state({5, 4}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state({5, 6}) == charge_state::NEUTRAL);
    ...
    SECTION("increase charge index of layout")
    {
        ...
        CHECK(charge_layout.get_charge_state_by_index(0) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state_by_index(1) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state_by_index(2) == charge_state::NEUTRAL);
        charge_layout.assign_charge_index(0);
        CHECK(charge_layout.get_charge_state_by_index(0) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state_by_index(1) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state_by_index(2) == charge_state::NEGATIVE);

        charge_distribution_surface charge_layout_quickexact{lyt, simulation_parameters{2}, charge_state::NEGATIVE};
        charge_layout_quickexact.set_simulation_engine(engine::QUICKEXACT);
        ...
        CHECK(charge_layout_quickexact.get_charge_state_by_index(0) == charge_state::NEGATIVE);
        CHECK(charge_layout_quickexact.get_charge_state_by_index(1) == charge_state::NEGATIVE);
        CHECK(charge_layout_quickexact.get_charge_state_by_index(2) == charge_state::NEUTRAL);
    }
    ...
    SECTION("charge distribution defined by a given charge index and vs and dependent-cell")
    {
        ...
        charge_distribution_surface charge_layout{lyt, simulation_parameters{3, -0.25}, charge_state::NEGATIVE};
        ...
        charge_layout.assign_charge_state({5, 5}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({6, 5}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({7, 5}, charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_index_of_sub_layout() == 8);

        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({6, 5}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({7, 5}) == charge_state::POSITIVE);

        charge_layout.assign_charge_index(0);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({6, 5}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({7, 5}) == charge_state::NEGATIVE);
        ...
        // detached.
        charge_layout.assign_charge_state({5, 5}, charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_index_and_base().first == 0);
        CHECK(charge_layout.get_charge_index_of_sub_layout() == 0);
        ...
        charge_layout.assign_charge_state({6, 5}, charge_state::POSITIVE, charge_index_mode::KEEP_CHARGE_INDEX);
        CHECK(charge_layout.get_charge_state({6, 5}) == charge_state::POSITIVE);
        ...
        CHECK(charge_layout.get_charge_index_of_sub_layout() == 8);
        charge_layout.set_simulation_engine(engine::EXGS);
    ...
    SECTION("cover behavior of dependent_cell in and outside the sublayout (positively charged SiDBs)")
    {
        ...
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}, charge_state::NEGATIVE};
        charge_layout.assign_dependent_cell({5, 5});
        ...
        charge_distribution_surface charge_layout_dependent_cell_not_in_sublayout{lyt, simulation_parameters{},
        ...
        charge_layout_dependent_cell_not_in_sublayout.assign_charge_state({5, 5}, charge_state::POSITIVE);
        charge_layout_dependent_cell_not_in_sublayout.assign_charge_state({6, 5}, charge_state::POSITIVE);
        charge_layout_dependent_cell_not_in_sublayout.assign_charge_state({7, 5}, charge_state::POSITIVE);
        charge_layout_dependent_cell_not_in_sublayout.assign_charge_state({10, 8}, charge_state::NEUTRAL);
    ...
    SECTION("charge index to charge distribution")
    {
        ...
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}, charge_state::NEGATIVE};
        charge_layout.set_simulation_engine(engine::QUICKEXACT);
        ...
        CHECK(charge_layout.get_charge_state({6, 5}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({7, 5}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({10, 4}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_index_of_sub_layout() == 0);
        CHECK(charge_layout.get_charge_index_and_base().first == 0);

        charge_layout.assign_charge_state({5, 5}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({10, 4}, charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({10, 4}) == charge_state::POSITIVE);
    ...
    SECTION("assign and read out charge states")
    {
        ...
        lyt.assign_cell_type({5, 5}, TestType::cell_type::NORMAL);
        lyt.assign_cell_type({5, 6}, TestType::cell_type::NORMAL);
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}};
        charge_layout.assign_charge_state({5, 4}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({5, 5}, charge_state::NEUTRAL);
        charge_layout.assign_charge_state({5, 6}, charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 4}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state({5, 6}) == charge_state::NEGATIVE);

        charge_layout.assign_charge_state_by_index(0, charge_state::NEUTRAL);
        charge_layout.assign_charge_state_by_index(1, charge_state::NEUTRAL);
        charge_layout.assign_charge_state_by_index(2, charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state_by_index(0) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state_by_index(1) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state_by_index(2) == charge_state::NEGATIVE);

        CHECK(charge_layout.get_charge_state({7, 6}) == charge_state::NONE);

        charge_layout.assign_charge_state_by_index(0, charge_state::NEUTRAL);
        charge_layout.assign_charge_state_by_index(1, charge_state::POSITIVE);
        charge_layout.assign_charge_state_by_index(2, charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state_by_index(0) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state_by_index(1) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state_by_index(2) == charge_state::POSITIVE);

        // check if charge state for empty cell is 'NONE'
        CHECK(charge_layout.get_charge_state_by_index(3) == charge_state::NONE);
    }
    ...
    SECTION("assign different charge states to the same cell")
    {
        ...
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}};
        charge_layout.assign_charge_state({5, 1}, charge_state::NEUTRAL);
        charge_layout.assign_charge_state({5, 1}, charge_state::NONE);
        charge_layout.assign_charge_state({5, 1}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({5, 1}, charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 1}) == charge_state::NEGATIVE);
    }
    ...
    SECTION("overwrite an assigned charge state")
    {
        ...
        charge_layout.assign_all_charge_states(charge_state::POSITIVE);
        ...
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 6}) == charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 1}) == charge_state::NONE);

        charge_layout.assign_all_charge_states(charge_state::POSITIVE);

        // all SiDBs' charge states are set to neutral
        charge_layout.assign_all_charge_states(charge_state::NEUTRAL);
        ...
        charge_layout.assign_all_charge_states(charge_state::NEGATIVE);
    ...
    SECTION("overwrite the charge state")
    {
        ...
        charge_layout.assign_charge_state({5, 4}, charge_state::NONE);
        charge_layout.assign_charge_state({5, 5}, charge_state::NONE);
        charge_layout.assign_charge_state({5, 6}, charge_state::POSITIVE);
        CHECK(charge_layout.get_charge_state({5, 4}) == charge_state::NONE);
        CHECK(charge_layout.get_charge_state({5, 5}) == charge_state::NONE);
        CHECK(charge_layout.get_charge_state({5, 6}) == charge_state::POSITIVE);
    }
    ...
    SECTION("set physical simulation parameters")
    {
        ...
        charge_layout.assign_physical_parameters(simulation_parameters{2, -0.2});
        ...
        charge_layout.assign_physical_parameters(simulation_parameters{3, -0.4, 5.1, 5.5});
    ...
    SECTION("Distance matrix")
    {
        ...
        CHECK_THAT(charge_layout.get_nm_distance_between_sidbs({0, 0, 0}, {1, 0, 0}),
                   Catch::Matchers::WithinAbs((lattice_100::LAT_A * 0.1), ERROR_MARGIN));
        ...
        CHECK_THAT(charge_layout.get_nm_distance_between_sidbs({0, 0, 0}, {1, 1, 1}),
                   Catch::Matchers::WithinAbs(
                       std::hypot(lattice_100::LAT_A * 0.1, lattice_100::LAT_B * 0.1 + lattice_100::LAT_C.second * 0.1),
                       ERROR_MARGIN));
    ...
    SECTION("Potential matrix")
    {
        ...
        CHECK_THAT(charge_layout.get_chargeless_potential_between_sidbs({1, 8, 0}, {0, 0, 0}),
                   Catch::Matchers::WithinAbs(0.01219448987342317, ERROR_MARGIN));
    ...
    SECTION("Local Potential")
    {
        ...
        lyt.assign_cell_type({1, 10, 1}, TestType::cell_type::NORMAL);
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}};
        charge_layout.assign_charge_state({0, 0, 0}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({1, 8, 0}, charge_state::POSITIVE);
        charge_layout.assign_charge_state({1, 10, 1}, charge_state::POSITIVE);
        ...
        charge_layout.assign_charge_state({0, 0, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({1, 8, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({1, 10, 1}, charge_state::NEGATIVE);
        ...
        charge_layout.foreach_cell(
            [&charge_layout](const auto& c)
            {
                const auto p = charge_layout.get_local_potential(c);
                REQUIRE(p.has_value());
                CHECK_THAT(p.value(), Catch::Matchers::WithinAbs(0.0, ERROR_MARGIN));
            });
    }
    ...
    SECTION("Electrostatic potential energy of the charge configuration")
    {
        ...
        charge_layout.assign_charge_state({0, 0, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({1, 0, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({1, 1, 1}, charge_state::NEGATIVE);
        ...
        charge_layout.update_local_internal_potential();
        charge_layout.recompute_electrostatic_potential_energy();
        CHECK_THAT(charge_layout.get_electrostatic_potential_energy(), Catch::Matchers::WithinAbs(0.0, ERROR_MARGIN));
    ...
    SECTION("Physical validity check under different physical parameters")
    {
        ...
        charge_distribution_surface charge_layout{layout, simulation_parameters{}};
        CHECK(charge_layout.get_charge_state({0, 0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({0, 2, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({4, 1, 1}) == charge_state::NEGATIVE);
        ...
        charge_layout.assign_physical_parameters(simulation_parameters{2, 0.0});
        CHECK(charge_layout.is_physically_valid());
    }
    ...
    SECTION("Physical validity check, far distance of SiDBs, all NEGATIVE")
    {
        ...
        charge_distribution_surface charge_layout_five{layout, simulation_parameters{}};
        CHECK(charge_layout_five.get_charge_state({0, 0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout_five.get_charge_state({0, 2, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout_five.get_charge_state({4, 1, 1}) == charge_state::NEGATIVE);
        ...
        charge_layout_five.assign_charge_state({4, 1, 1}, charge_state::POSITIVE);
        CHECK(charge_layout_five.get_charge_state({4, 1, 1}) == charge_state::POSITIVE);
        ...
        CHECK(charge_layout_five.get_charge_state({0, 0, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout_five.get_charge_state({0, 2, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout_five.get_charge_state({4, 1, 1}) == charge_state::NEUTRAL);
    }
    ...
    SECTION("Physical validity check, small distance, not all can be negatively charged anymore")
    {
        ...
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}};
        charge_layout.assign_charge_state({1, 0, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({0, 2, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({0, 2, 1}, charge_state::NEGATIVE);
    ...
    SECTION("apply external voltage at two cells")
    {
        ...
        charge_distribution_surface charge_layout_new{lyt_new, params, charge_state::NEUTRAL};
        ...
        CHECK_THAT(charge_layout_new.get_local_potential({0, 0, 1}).value() + 0.5,
                   Catch::Matchers::WithinAbs(0, ERROR_MARGIN));
        ...
        CHECK_THAT(charge_layout_new.get_local_potential({10, 5, 1}).value(),
                   Catch::Matchers::WithinAbs(0.0, ERROR_MARGIN));
        charge_layout_new.assign_all_charge_states(charge_state::POSITIVE);
        charge_layout_new.update_after_charge_change();
        CHECK(charge_layout_new.get_charge_state({0, 0, 1}) == charge_state::POSITIVE);
        CHECK(charge_layout_new.get_charge_state({1, 3, 0}) == charge_state::POSITIVE);
        CHECK(charge_layout_new.get_charge_state({10, 5, 1}) == charge_state::POSITIVE);
        ...
        charge_layout_new.assign_all_charge_states(charge_state::NEUTRAL);
        charge_layout_new.update_after_charge_change();
        ...
        charge_layout_new.assign_all_charge_states(charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();
    ...
    SECTION("apply homogenous external voltage to layout")
    {
        ...
        charge_distribution_surface charge_layout{lyt, params, charge_state::NEUTRAL};
        ...
        charge_distribution_surface charge_layout_new{lyt, params, charge_state::NEGATIVE};
        charge_layout_new.assign_charge_state({3, 0, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({5, 0, 0}, charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();
        ...
        charge_layout_new.assign_charge_state({0, 0, 0}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({3, 0, 0}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({5, 0, 0}, charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();
        CHECK(charge_layout_new.is_physically_valid());
    }
    ...
    SECTION("no external voltage given")
    {
        ...
        const charge_distribution_surface charge_layout_new{lyt_new, params, charge_state::NEUTRAL};
    ...
    SECTION("Seven randomly distributed DB | checking for physical validity")
    {
        ...
        charge_layout_new.assign_charge_state({1, 3, 0}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({3, 3, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({4, 3, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({6, 3, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({7, 3, 0}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({6, 10, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({7, 10, 0}, charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();
    ...
    {
        TestType                    lyt_new{};
        const simulation_parameters params{3, -0.32, 5.0};
        ...
        charge_layout_new.assign_charge_state({0, 0, 1}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({1, 3, 0}, charge_state::POSITIVE);
        charge_layout_new.assign_charge_state({10, 5, 1}, charge_state::NEUTRAL);

        CHECK(charge_layout_new.get_charge_state({0, 0, 1}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({1, 3, 0}) == charge_state::POSITIVE);
        CHECK(charge_layout_new.get_charge_state({10, 5, 1}) == charge_state::NEUTRAL);
    ...
    SECTION("adding dependent cell")
    {
        ...
        charge_distribution_surface charge_layout_new{lyt_new, params, charge_state::NEGATIVE};
        charge_layout_new.assign_dependent_cell({10, 4, 1});

        charge_layout_new.assign_charge_state({0, 3, 1}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({1, 3, 0}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({10, 4, 1}, charge_state::NEUTRAL);

        CHECK(charge_layout_new.get_charge_state({0, 3, 1}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({1, 3, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({10, 4, 1}) == charge_state::NEUTRAL);
        charge_layout_new.update_after_charge_change();
        ...
        CHECK(charge_layout_new.get_charge_state({10, 4, 1}) == charge_state::NEGATIVE);
    }
    ...
    SECTION("dependent cell in alignment with the base number")
    {
        ...
        charge_layout_new.assign_charge_state({0, 1, 1}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({0, 1, 0}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({2, 1, 1}, charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();

        charge_layout_new.update_charge_state_of_dependent_cell();
        CHECK(charge_layout_new.get_charge_state({0, 1, 1}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({0, 1, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({2, 1, 1}) == charge_state::NEGATIVE);
        ...
        CHECK(charge_layout_new.get_charge_state({0, 1, 1}) == charge_state::POSITIVE);
        CHECK(charge_layout_new.get_charge_state({0, 1, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({2, 1, 1}) == charge_state::NEGATIVE);
    }
    ...
    SECTION("adding dependent cell and compare local potential and system energy")
    {
        ...
        charge_layout_new.assign_charge_state({0, 3, 1}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({1, 3, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({10, 4, 1}, charge_state::NEUTRAL);
        charge_layout_new.update_after_charge_change();
        ...
        charge_layout_new.update_charge_state_of_dependent_cell();
        CHECK(charge_layout_new.get_charge_state({0, 3, 1}) == charge_state::NEGATIVE);
        CHECK(charge_layout_new.get_charge_state({1, 3, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout_new.get_charge_state({10, 4, 1}) == charge_state::NEGATIVE);
        ...
        charge_layout_new.assign_charge_state({0, 3, 1}, charge_state::NEGATIVE);
        charge_layout_new.assign_charge_state({1, 3, 0}, charge_state::NEUTRAL);
        charge_layout_new.assign_charge_state({10, 4, 1}, charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();
        ...
        CHECK_THAT(system_energy_first - system_energy_second, Catch::Matchers::WithinAbs(0, ERROR_MARGIN));
    }
    ...
    SECTION("Physical validity check after dependent cell is updated")
    {
        ...
        CHECK(charge_layout.get_charge_state({0, 0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({3, 0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 0, 0}) == charge_state::NEGATIVE);
        CHECK(!charge_layout.is_physically_valid());
    ...
    SECTION("check charge index")
    {
        ...
        charge_distribution_surface charge_layout{lyt, params, charge_state::NEGATIVE};
        charge_layout.assign_dependent_cell({3, 0, 0});
        CHECK(charge_layout.get_charge_state({0, 0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({3, 0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_layout.get_charge_state({5, 0, 0}) == charge_state::NEGATIVE);
        ...
        CHECK(charge_layout.get_charge_state({0, 0, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state({5, 0, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_index_and_base().first == 3);
    }
    ...
    SECTION("check charge index")
    {
        ...
        CHECK(charge_layout.get_charge_state({3, 0, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_state({5, 0, 0}) == charge_state::NEUTRAL);
        CHECK(charge_layout.get_charge_index_and_base().first == 3);
    }
    ...
    SECTION("Y-shaped SiDB OR gate with input 01 and global external potential (high)")
    {
        ...
        const simulation_parameters params{3, -0.28};
        charge_distribution_surface charge_lyt_first{lyt, params, charge_state::POSITIVE};
    ...
    SECTION("detecting DBs which could be positively charged due to maximal band bending")
    {
        ...
        const simulation_parameters params{3, -0.28};
        charge_distribution_surface charge_lyt_first{lyt, params, charge_state::NEGATIVE};
    ...
    SECTION("detecting DBs which could be positively charged due to maximal band bending")
    {
        ...
        const simulation_parameters params{3, -0.32};
        charge_distribution_surface charge_lyt_first{lyt, params, charge_state::NEGATIVE};
        charge_lyt_first.assign_charge_state({0, 0, 0}, charge_state::NEGATIVE);
        charge_lyt_first.assign_charge_state({4, 0, 0}, charge_state::NEUTRAL);
        charge_lyt_first.assign_charge_state({6, 0, 0}, charge_state::NEGATIVE);
...

{
    ...
    SECTION("Assign charged defect")
    {
        ...
        charge_layout.add_sidb_defect_to_potential_landscape(
            {5, 6}, defect{defect_type::UNKNOWN, -1, charge_layout.get_simulation_params().epsilon_r,
    ...
    SECTION("Assign neutral defect")
    {
        ...
        charge_distribution_surface charge_layout{lyt, simulation_parameters{}};
        charge_layout.add_sidb_defect_to_potential_landscape(
            {5, 6}, defect{defect_type::UNKNOWN, 0, charge_layout.get_simulation_params().epsilon_r,
                           charge_layout.get_simulation_params().lambda_tf});

        REQUIRE(charge_layout.get_local_potential({5, 4}).has_value());
        CHECK_THAT(charge_layout.get_local_potential({5, 4}).value(), Catch::Matchers::WithinAbs(0, ERROR_MARGIN));
    }
    ...
    SECTION("perturber is replaced by an equivalent defect")
    {
        ...
        charge_distribution_surface charge_layout_new{lyt, simulation_parameters{}};
        charge_layout_new.add_sidb_defect_to_potential_landscape(
            {5, 6}, defect{defect_type::UNKNOWN, -1, charge_layout_new.get_simulation_params().epsilon_r,
...

{
    ...
    SECTION("assign defect | negative defect")
    {
        ...
        charge_layout_new.add_sidb_defect_to_potential_landscape(
            {5, 1, 1}, defect{defect_type::UNKNOWN, -1, charge_layout_new.get_simulation_params().epsilon_r,
                              charge_layout_new.get_simulation_params().lambda_tf});
    ...
    SECTION("assign defect and perturber")
    {
        ...
        charge_layout_new.assign_charge_state({10, 5, 1}, charge_state::NEGATIVE);
        charge_layout_new.update_after_charge_change();

        charge_layout_new.add_sidb_defect_to_potential_landscape(
            {-10, 5, 1}, defect{defect_type::UNKNOWN, 1, charge_layout_new.get_simulation_params().epsilon_r,
                                charge_layout_new.get_simulation_params().lambda_tf});
    ...
    SECTION("layout with perturber |assigning and erasing defect")
    {
        ...
        charge_distribution_surface charge_layout{lyt_new, params, charge_state::NEUTRAL};

        REQUIRE(charge_layout.get_local_potential({0, 0, 0}).has_value());
        REQUIRE(charge_layout.get_local_potential({10, 5, 1}).has_value());

        charge_layout.assign_charge_state({10, 5, 1}, charge_state::NEGATIVE);
        charge_layout.update_after_charge_change();
        charge_layout.add_sidb_defect_to_potential_landscape(
            {-10, 5, 1}, defect{defect_type::UNKNOWN, 1, charge_layout.get_simulation_params().epsilon_r,
                                charge_layout.get_simulation_params().lambda_tf});

        CHECK_THAT(charge_layout.get_local_potential({0, 0, 0}).value(), Catch::Matchers::WithinAbs(0.0, ERROR_MARGIN));
    ...
    SECTION("experiments with defects")
    {
        ...
        charge_layout.add_sidb_defect_to_potential_landscape(
            {-4, 0, 0}, defect{defect_type::UNKNOWN, -1, charge_layout.get_simulation_params().epsilon_r,
                               charge_layout.get_simulation_params().lambda_tf});
        ...
        charge_layout.add_sidb_defect_to_potential_landscape(
            {-4, 0, 0}, defect{defect_type::UNKNOWN, 0, charge_layout.get_simulation_params().epsilon_r,
                               charge_layout.get_simulation_params().lambda_tf});
    ...
    SECTION("assign defect on DB position which is not allowed")
    {
        ...
        charge_layout.add_sidb_defect_to_potential_landscape(
            {0, 0, 0}, defect{defect_type::UNKNOWN, -1, charge_layout.get_simulation_params().epsilon_r,
                              charge_layout.get_simulation_params().lambda_tf});
    ...
    SECTION("assign defects with different screening lengths")
    {
        ...
        charge_layout.add_sidb_defect_to_potential_landscape(
            {8, 0, 0}, defect{defect_type::UNKNOWN, -1, charge_layout.get_simulation_params().epsilon_r,
                              charge_layout.get_simulation_params().lambda_tf});
        ...
        CHECK_THAT(charge_layout.get_local_potential({0, 0, 0}).value(), Catch::Matchers::WithinAbs(0, ERROR_MARGIN));
        CHECK_THAT(charge_layout.get_local_potential({3, 0, 0}).value(), Catch::Matchers::WithinAbs(0, ERROR_MARGIN));
        CHECK_THAT(charge_layout.get_local_potential({5, 0, 0}).value(), Catch::Matchers::WithinAbs(0, ERROR_MARGIN));
    }
}

TEST_CASE("Assign charge index", "[charge-distribution-surface]")
{
    using TestType = lattice<lattice_100, sidb_cell_clk_lyt_siqad>;
    TestType lyt{};
    ...
    SECTION("Base 2")
    {
        charge_distribution_surface<TestType> charge_lyt{lyt, simulation_parameters{2}};

        charge_lyt.assign_charge_index(0);
        CHECK(charge_lyt.get_charge_state({0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_lyt.get_charge_state({0, 1}) == charge_state::NEGATIVE);

        charge_lyt.assign_charge_index(1);
        CHECK(charge_lyt.get_charge_state({0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_lyt.get_charge_state({0, 1}) == charge_state::NEUTRAL);

        charge_lyt.assign_charge_index(3);
        CHECK(charge_lyt.get_charge_state({0, 0}) == charge_state::NEUTRAL);
        CHECK(charge_lyt.get_charge_state({0, 1}) == charge_state::NEUTRAL);
    ...
    SECTION("Base 3")
    {
        charge_distribution_surface<TestType> charge_lyt{lyt, simulation_parameters{3}};
        ...
        CHECK(charge_lyt.get_charge_state({0, 0}) == charge_state::NEGATIVE);
        CHECK(charge_lyt.get_charge_state({0, 1}) == charge_state::POSITIVE);

        charge_lyt.assign_charge_index(8);
        CHECK(charge_lyt.get_charge_state({0, 0}) == charge_state::POSITIVE);
        CHECK(charge_lyt.get_charge_state({0, 1}) == charge_state::POSITIVE);
                   ...
                   sidb_100_cell_clk_lyt_siqad, cds_sidb_100_cell_clk_lyt_siqad)
{
    ...
    const auto                                 sim_params = simulation_parameters{};
    charge_distribution_surface<decltype(lyt)> charge_lyt{lyt, sim_params};

    charge_lyt.assign_charge_state({0, 1, 0}, charge_state::NEUTRAL);
    ...
    defect_surface<decltype(lyt)> defect_lyt{lyt};
    defect_lyt.assign_cell_type({5, 1, 0}, sidb_cell_clk_lyt_siqad::cell_type::EMPTY);
    defect_lyt.assign_defect({5, 1, 0}, defect{defect_type::DB, -1, sim_params.epsilon_r, sim_params.lambda_tf});

    charge_distribution_surface<decltype(defect_lyt)> charge_lyt_defect{defect_lyt, sim_params};

    charge_lyt_defect.assign_charge_state({0, 1, 0}, charge_state::NEUTRAL);
...
TEST_CASE("Tests for Si-111 lattice orientation", "[charge-distribution-surface]")
{
    ...
    SECTION("Electrostatic potential energy of the charge configuration")
    {
        ...
        CHECK_THAT(charge_layout.get_nm_distance_between_sidbs({0, 0, 0}, {2, 0, 0}),
                   Catch::Matchers::WithinAbs(1.33, ERROR_MARGIN));

        charge_layout.assign_charge_state({0, 0, 0}, charge_state::NEGATIVE);
        charge_layout.assign_charge_state({2, 0, 0}, charge_state::NEGATIVE);

        // system energy is zero when all SiDBs are positively charged.
        charge_layout.update_local_internal_potential();
        charge_layout.recompute_electrostatic_potential_energy();
        CHECK_THAT(charge_layout.get_electrostatic_potential_energy(),
                   Catch::Matchers::WithinAbs(0.14818, ERROR_MARGIN));
