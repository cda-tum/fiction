...
TEST_CASE("Generate MolQCA layout in SVG", "[write-mol-qca-layout-svg]")
{
    ...
    SECTION("detailed mode")
    {
        ...
        CHECK(contains(svg, "viewBox=\"0 0 102.5 33.5\""));
        CHECK(contains(svg, "Generated by"));
        CHECK(contains(svg, "fill:#ffe700"));
        CHECK(contains(svg, "fill:#cccccc"));
        CHECK(contains(svg, "fill:#4a4a4a"));
        CHECK(contains(svg, "fill:#f800ff"));
    ...
    SECTION("simple mode")
    {
        ...
        CHECK(contains(svg, "ry=\"0.75\""));
        CHECK_FALSE(contains(svg, "C0"));
        CHECK_FALSE(contains(svg, "C1"));
        CHECK_FALSE(contains(svg, "left molecule"));
    }
...
TEST_CASE("Reject unsupported MolQCA SVG cell types", "[write-mol-qca-layout-svg]")
{
    ...
    CHECK_THROWS_AS(write_mol_qca_layout_svg(layout, layout_stream),
                    unsupported_cell_type_exception<offset::ucoord_t>);
}
...
TEST_CASE("Reject invalid MolQCA SVG output files", "[write-mol-qca-layout-svg]")
{
    ...
    CHECK_THROWS_AS(write_mol_qca_layout_svg(layout, "/this/path/does/not/exist/mol_qca.svg"),
