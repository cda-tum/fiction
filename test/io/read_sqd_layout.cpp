...
TEST_CASE("Read empty SQD layout", "[sqd]")
{
    ...
    check(read_sqd_layout<sidb::primitives::lattice<sidb::primitives::lattice_100, sidb_layout>>(layout_stream));
}
...
TEST_CASE("Read single-dot SQD layout", "[sqd]")
{
    ...
    const auto check = [](const auto& lyt)
    {
        ...
        CHECK(lyt.y() == 0);
        CHECK(lyt.area() == 1);
        CHECK(lyt.get_cell_type({0, 0}) == sidb::technology::cell_type::NORMAL);
    };

    using sidb_layout =
        layouts::cell_level_layout<sidb::technology,
                                   layouts::clocked_layout<layouts::cartesian_layout<layouts::coords::offset>>>;
    check(read_sqd_layout<sidb::primitives::lattice<sidb::primitives::lattice_111, sidb_layout>>(layout_stream));
}
...
TEST_CASE("Read multi-dot SQD layout", "[sqd]")
{
    ...
    SECTION("Fiction coordinates")
    {
        ...
        CHECK(layout.get_cell_type({0, 0}) == sidb::technology::cell_type::NORMAL);
        CHECK(layout.get_cell_type({0, 1}) == sidb::technology::cell_type::NORMAL);
        CHECK(layout.get_cell_type({2, 4}) == sidb::technology::cell_type::NORMAL);
        CHECK(layout.get_cell_type({2, 5}) == sidb::technology::cell_type::NORMAL);
    }
    ...
    SECTION("SiQAD coordinates")
    {
        ...
        const auto layout =
            read_sqd_layout<sidb::primitives::lattice<sidb::primitives::lattice_100, siqad_layout>>(layout_stream);

        CHECK(layout.x() == 2);
        CHECK(layout.y() == 2);
        CHECK(layout.z() == 1);

        CHECK(layout.get_cell_type({0, 0, 0}) == sidb::technology::cell_type::NORMAL);
        CHECK(layout.get_cell_type({0, 0, 1}) == sidb::technology::cell_type::NORMAL);
        CHECK(layout.get_cell_type({2, 2, 0}) == sidb::technology::cell_type::NORMAL);
        CHECK(layout.get_cell_type({2, 2, 1}) == sidb::technology::cell_type::NORMAL);
    }
}
...
TEST_CASE("Read multi-dot SQD layout with cell type definitions", "[sqd]")
{
    ...
    CHECK(layout.get_cell_type({0, 0}) == sidb::technology::cell_type::INPUT);
    CHECK(layout.get_cell_type({0, 1}) == sidb::technology::cell_type::OUTPUT);
    CHECK(layout.get_cell_type({2, 4}) == sidb::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({2, 5}) == sidb::technology::cell_type::NORMAL);
    CHECK(layout.get_cell_type({1, 3}) == sidb::technology::cell_type::LOGIC);
}
...
TEST_CASE("Read multiple defects SQD layout", "[sqd]")
{
    ...
    }
    {
        ...
        CHECK(defect2.type == sidb::model::defect_type::MISSING_DIMER);
    ...
    }
    {
        ...
        CHECK(defect1.epsilon_r == 5.6);
        CHECK(defect1.lambda_tf == 5.0);
        CHECK(defect2.type == sidb::model::defect_type::ETCH_PIT);
        CHECK(defect2.charge == -1);
        CHECK(defect2.epsilon_r == 5.6);
        CHECK(defect2.lambda_tf == 5.0);
        CHECK(defect3.type == sidb::model::defect_type::ETCH_PIT);
        CHECK(defect3.charge == -1);
        CHECK(defect3.epsilon_r == 5.6);
        CHECK(defect3.lambda_tf == 5.0);
        CHECK(defect4.type == sidb::model::defect_type::ETCH_PIT);
...
TEST_CASE("Read multi-dot SQD layout with multi-cell defect", "[sqd]")
{
    ...
    layout.foreach_sidb_defect(
        [&defect_positions](const auto& cd)
        {
            ...
            CHECK(defect.type == sidb::model::defect_type::DB);
