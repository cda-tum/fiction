...
TEST_CASE("Read empty SQD layout", "[sqd]")
{
    ...
    check(read_sqd_layout<sidb_lattice<sidb_layout>>(layout_stream));
}
...
TEST_CASE("Read multi-dot SQD layout", "[sqd]")
{
    ...
    {
        using sidb_layout = cell_level_layout<sidb_technology, clocked_layout<cartesian_layout<offset::ucoord_t>>>;
        const auto layout = read_sqd_layout<sidb_lattice<sidb_layout>>(layout_stream);
    ...
    SECTION("SiQAD coordinates")
    {
        ...
        const auto layout = read_sqd_layout<sidb_lattice<sqd_layout>>(layout_stream);
...
TEST_CASE("In-place SQD reader with ignored defects", "[sqd]")
{
    ...
    sidb_lattice<sidb_layout, sidb_111_lattice> lattice_layout{layout};
...
TEST_CASE("SQD parsing error: missing <siqad> element", "[sqd]")
{
    ...
    CHECK_THROWS_AS(read_sqd_layout<sidb_lattice<sidb_layout>>(layout_stream), sqd_parsing_error);
}
