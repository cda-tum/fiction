        ...
        layouts::clocked_layout<layouts::hexagonal_layout<layouts::coords::offset, layouts::even_column_hex>>>))
{
    ...
    const auto check = [](const auto& lyt)
    {
        ...
        CHECK(lyt.get_sidb_defect({0, 0}).type == sidb::model::defect_type::NONE);
    };
        ...
        layouts::clocked_layout<layouts::hexagonal_layout<layouts::coords::offset, layouts::even_column_hex>>>))
{
    ...
    lyt.foreach_cell(
        [&lyt](const auto& c)
        {
            ...
            CHECK(lyt.get_sidb_defect(c).type == sidb::model::defect_type::NONE);
        });
}
        ...
        layouts::clocked_layout<layouts::hexagonal_layout<layouts::coords::offset, layouts::even_column_hex>>>))
{
    ...
    CHECK(lyt.get_sidb_defect({0, 0}).type == sidb::model::defect_type::NONE);
    CHECK(lyt.get_sidb_defect({1, 0}).type == sidb::model::defect_type::DB);
    CHECK(lyt.get_sidb_defect({2, 0}).type == sidb::model::defect_type::SI_VACANCY);
    CHECK(lyt.get_sidb_defect({3, 0}).type == sidb::model::defect_type::DIHYDRIDE_PAIR);
    CHECK(lyt.get_sidb_defect({0, 1}).type == sidb::model::defect_type::SINGLE_DIHYDRIDE);
    CHECK(lyt.get_sidb_defect({1, 1}).type == sidb::model::defect_type::ONE_BY_ONE);
    CHECK(lyt.get_sidb_defect({2, 1}).type == sidb::model::defect_type::THREE_BY_ONE);
    CHECK(lyt.get_sidb_defect({3, 1}).type == sidb::model::defect_type::SILOXANE);
    CHECK(lyt.get_sidb_defect({0, 2}).type == sidb::model::defect_type::RAISED_SI);
    CHECK(lyt.get_sidb_defect({1, 2}).type == sidb::model::defect_type::ETCH_PIT);
    CHECK(lyt.get_sidb_defect({2, 2}).type == sidb::model::defect_type::MISSING_DIMER);
    CHECK(lyt.get_sidb_defect({3, 2}).type == sidb::model::defect_type::NONE);
}
