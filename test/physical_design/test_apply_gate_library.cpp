...
TEST_CASE("Gate-library application assigns synchronization to clock zones", "[apply-gate-library]")
{
    const auto check_zone_delay = [](const auto& cells, const tile<cart_gate_clk_lyt>& delayed_tile)
    {
        cells.foreach_coordinate(
            [&cells, &delayed_tile](const auto& c)
            {
                const auto delayed = c.x / qca_one_library::gate_x_size() == delayed_tile.x &&
                                     c.y / qca_one_library::gate_y_size() == delayed_tile.y;
                CHECK(cells.get_synchronization_element(c) == (delayed ? 2 : 0));
            });
        CHECK(cells.num_se() == 1);
    };
    ...
    SECTION("Wire")
    {
        ...
        check_zone_delay(apply_gate_library<qca_one_library>(layout), {1, 0});
    }
    SECTION("Crossing")
    {
        // the ground tile carries the delay, as in layouts generated by exact; the crossing wire shares its clock zone
        auto layout = blueprints::crossing_layout<cart_gate_clk_lyt>();
        layout.assign_synchronization_element({2, 1}, 2);

        const auto cells = apply_gate_library<qca_one_library>(layout);

        check_zone_delay(cells, {2, 1});

        auto crossing_cells = 0u;
        cells.foreach_cell(
            [&cells, &crossing_cells](const auto& c)
            {
                if (c.z == 1)
                {
                    ++crossing_cells;
                    CHECK(cells.is_synchronization_element(c));
                }
            });
        CHECK(crossing_cells > 0);
    }
}
...
TEST_CASE("On-the-fly SiDB fanout gate", "[apply-gate-library]")
{
    ...
    CHECK(std::ranges::any_of(
        fanout_gate, [](const auto& row)
        { return std::ranges::any_of(row, [](const auto cell) { return cell == sidb::dot_tag::LOGIC; }); }));
    ...
    CHECK(std::ranges::any_of(
        fanout_gate_with_defect, [](const auto& row)
        { return std::ranges::any_of(row, [](const auto cell) { return cell == sidb::dot_tag::LOGIC; }); }));
}
...
TEST_CASE("Apply molecular QCA gate library end-to-end", "[apply-gate-library]")
{
    ...
    const auto cell_layout = apply_gate_library<mol_qca::sim7_mol_library>(layout);
